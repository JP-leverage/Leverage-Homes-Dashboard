...
const THEMES = {
  light: {
    ...
    gridLine: "rgba(15,46,74,0.045)", zebra: "rgba(15,46,74,0.045)",
  ...
  dark: {
    ...
    gridLine: "rgba(255,255,255,0.028)", zebra: "rgba(255,255,255,0.04)",
    shadow: "none",
...
const WORKBOOKS = {
  ...
  marketing_pt2: { id: "189aqyVy3bg4enmJNDo3UQMcAbTc3R2Xi65d2XIXup6M", title: "Homes Dashboard PT2 (Marketing)" },
...
const DATASETS = {
  ...
  arip_entered: {
    workbook: "opportunities",
    require: ["New Value", "Opportunity Owner", "Acquisition Manager"], exclude: [], tabInclude: /Opps\s*-\s*ARIP\s*-\s*(YTD|All Time)/i,
    schema: { id: "Opportunity ID", name: "Opportunity Name", owner: "Opportunity Owner", acqManager: "Acquisition Manager",
  ...
  leads: {
    workbook: "marketing",
    require: ["Lead ID", "Lead Source"], exclude: [], tabExclude: /^All leads|Reactivated|Converted/i,
    schema: { leadId: "Lead ID", account: "Company / Account", status: "Lead Status",
      ...
      oppId: ["oppId", "Converted Opportunity ID", "Converted Opportunity: Opportunity ID"] }, // lead → opp link for cohort attribution
  ...
  // Reactivations at event grain (one row per status change) — the cost panel counts only Dead → New Lead revives.
  mkt_revives: {
    workbook: "marketing",
    require: ["Lead ID", "Field / Event"], exclude: [], tabInclude: /Reactivated/i,
    schema: { leadId: "Lead ID", source: "Lead Source", oldValue: "Old Value", newValue: "New Value" },
    dedupe: (r) => `${r.leadId}|${r.date}|${r.oldValue}`, dateField: "date", dateCandidates: ["Edit Date"], repField: null,
  },
  // ── Marketing cost attribution — PT2 Marketing workbook (QuickBooks + Rippling imports). See MKT_COST.
  mkt_spend: { // QuickBooks "Vendor Expenses by Month" pivot: Vendor × month columns (+ Total). Kept raw and unpivoted in buildMktCostModel.
    workbook: "marketing_pt2",
    require: ["Vendor"], exclude: ["Vendor Type"], tabInclude: /Marketing Spend COGS/i, tabField: "__tab", passthrough: true,
    schema: {}, dedupe: null, dateField: null, repField: null,
  },
  mkt_vendor_map: { // Vendor → Lead Source(s) + Vendor Type (Software / Service = direct, Data = shared)
    workbook: "marketing_pt2",
    require: ["Vendor"], exclude: [], tabInclude: /Vendor x Lead Source/i,
    schema: { vendor: "Vendor", sources: ["Lead Source(s)", "Lead Source", "Lead Sources"], type: "Vendor Type" },
    dedupe: null, dateField: null, repField: null,
  },
  mkt_roster: { // Rippling roster — only identity + title are read (no DOB / comp / contact fields)
    workbook: "marketing_pt2",
    require: ["Employee - ID", "Title"], exclude: [], tabInclude: /Roster/i, tabExclude: /addition|manual/i,
    schema: { empId: "Employee - ID", name: "Employee", first: "Legal first name", last: "Legal last name", title: "Title", status: "Employment status" },
    dedupe: (r) => r.empId || null, dateField: null, repField: null,
  },
  mkt_roster_add: { // optional hand-kept tab for people not in Rippling (e.g. former callers): Legal first name, Legal last name, Title
    workbook: "marketing_pt2",
    require: ["Legal first name", "Legal last name", "Title"], exclude: [], tabInclude: /Roster.*(addition|manual)/i, optional: true,
    schema: { empId: "Employee - ID", name: "Employee", first: "Legal first name", last: "Legal last name", title: "Title" },
    dedupe: null, dateField: null, repField: null,
  },
  mkt_labor: { // QuickBooks P&L detail of contractor pay; Rippling journal memos carry the payee + pay period
    workbook: "marketing_pt2",
    require: ["Memo/Description", "Amount"], exclude: [], tabInclude: /Contractor Payments/i,
    schema: { account: "Distribution Account", txType: "Transaction Type", postDate: "Date", payee: "Name", memo: "Memo/Description", amount: "Amount" },
    dedupe: null, dateField: null, repField: null,
  },
  leads_claimed: {
    workbook: "leads_wb",
  ...
  closed_opps: {
    ...
    schema: { id: "Opportunity ID", owner: "Opportunity Owner", name: "Opportunity Name",
      ...
      source: "Lead Source", recordType: "Opportunity Record Type" },
    dedupe: (r) => (r.id != null && r.id !== "" ? String(r.id) : null), dateField: "closeDate",
    repField: "owner", repFields: ["owner", "acqManager", "acqManager2", "followUp"],
  },
  // Optional historical closed deals (e.g. "Closed Opps x All Time") — read only by the marketing cost panel to
  // unlock lifetime / trailing-12 windows; the Sales tab keeps using closed_opps (YTD). Same columns as the YTD tab.
  closed_opps_hist: {
    workbook: "opportunities_pt2",
    require: ["Opportunity ID", "Close Date"], exclude: [], tabInclude: /Closed Opps x (All Time|Last Year|20\d\d)/i,
    schema: { id: "Opportunity ID", name: "Opportunity Name", revenue: ["Total Forecasted Revenue", "Total Net Revenue", "Net Revenue"],
      closeDate: "Close Date", source: "Lead Source", recordType: "Opportunity Record Type" },
    dedupe: (r) => (r.id != null && r.id !== "" ? String(r.id) : null), dateField: "closeDate", repField: null, optional: true,
  },
  listing_pipeline: {
    workbook: "opportunities_pt2",
  ...
  // appt-set and UW-complete timestamps used for the Appt Set -> UW Complete duration. Date axis = appt set
  // ("Date/Time is Appt Set") — the window shows the deals UW was requested on (so pending/backlog rows, which
  // have no complete date yet, still fall in the window and count correctly).
  uw_cycle: {
    ...
    dateField: "date", dateCandidates: ["Date/Time is Appt Set"],
  ...
  uw_accuracy: {
    ...
    schema: {
      ...
      asIs: "As Is Valuation", asIsUw: ["As Is Valuation.1", "As Is Valuation (2)"],
      contractPrice: "Underlying Contract Price (Seller)", endBuyer: "Accepted Offer Amount (End Buyer)", arv: "ARV",
      // Preferred back-end basis: the front-end opp's ARV pulled directly onto the row by the report (no name
      // matching). Null today; lights up automatically if a "Front-End Opportunity: ARV" column is added.
      feArv: ["Front-End Opportunity: ARV", "Front-End Opportunity: After Repair Value", "Front-End ARV"],
      feId: ["Front-End Opportunity: Opportunity ID", "Front-End Opportunity ID"],
    },
...
async function loadAll(onProgress) {
  ...
  for (const key of keys) {
    ...
    store[key] = dedupe(ds.passthrough ? rows : normalize(rows, ds), ds.dedupe); // passthrough keeps every column (wide pivots)
    done++;
    onProgress && onProgress({ done, total, label: friendly(ds) });
    if (useGoogle && !rows.length && !ds.optional)
...
function buildSample() {
  ...
  for (let i = 0; i < 34; i++) {
    ...
    uw_accuracy.push({ "Opportunity ID": "006VI" + (400000 + i), "Opportunity Name": `${20 + i} Accuracy Rd, NJ`,
      ...
      "As Is Valuation": asIs, "As Is Valuation.1": asIs,
  ...
  uw_accuracy.push({ "Opportunity ID": "006VI444002", "Opportunity Name": "Back-end ~ " + feName, "Front-End Opportunity: Opportunity Name": feName,
    ...
    "Underwriter: Full Name": "", "Due Diligence Specialist": "", "As Is Valuation": "", "As Is Valuation.1": "",
    "Front-End Opportunity: ARV": 700000, "Front-End Opportunity: Opportunity ID": "006VI444001",
  ...
  // Marketing cost samples (PT2 Marketing shapes): a vendor×month pivot, the vendor map, a small roster and Rippling memos.
  const MON = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  const mkt_spend = [["Dialer Co", "Software", "Call Center", 9000], ["SMS Co", "Software", "Text Message Campaign", 2600], ["List Data Co", "Data", "Call Center, Text Message Campaign", 1500],
    ["Mail Co", "Service", "Direct Mail Campaign", 5000], ["Lead Vendor Co", "Service", "Pay Per Lead", 4000], ["New Vendor Co", "", "", 700]].map(([v, , , base]) => {
    const row = { Vendor: v, __tab: "Marketing Spend COGS x All Time" };
    for (let k = 0; k < 9; k++) { const d = new Date(REF.getFullYear(), REF.getMonth() - k, 1); row[`${MON[d.getMonth()]} ${d.getFullYear()}`] = Math.round(base * (0.7 + r() * 0.6)); }
    return row; });
  const mkt_vendor_map = [["Dialer Co", "Call Center", "Software"], ["SMS Co", "Text Message Campaign", "Software"], ["List Data Co", "Call Center, Text Message Campaign", "Data"],
    ["Mail Co", "Direct Mail Campaign", "Service"], ["Lead Vendor Co", "Pay Per Lead", "Service"]].map(([v, s, t]) => ({ Vendor: v, "Lead Source(s)": s, "Vendor Type": t }));
  const mkt_roster = [["E1", "Ana Cruz", "Ana", "Cruz", "Call Center Associate, Tier 1"], ["E2", "Ben Reyes", "Ben", "Reyes", "Call Center Associate, Tier 1"],
    ["E3", "Cara Lim", "Cara", "Lim", "Texting Agent, Tier 1"]].map(([id, n, f, l, t]) => ({ "Employee - ID": id, Employee: n, "Legal first name": f, "Legal last name": l, Title: t, "Employment status": "Active" }));
  const mkt_labor = [];
  ["Ana Maria Cruz", "Ben Santos Reyes", "Cara Lim", "Someone Else"].forEach((nm) => { for (let k = 0; k < 16; k++) {
    const st = new Date(REF); st.setDate(1); st.setMonth(st.getMonth() - Math.floor(k / 2)); if (k % 2) st.setDate(16);
    const en = new Date(st); if (k % 2) en.setMonth(en.getMonth() + 1, 0); else en.setDate(15);
    const post = new Date(en); post.setDate(post.getDate() + 3);
    mkt_labor.push({ "Distribution Account": "600100_International Contractor Expense", "Transaction Type": "Journal Entry", Date: iso(post), Amount: 400 + Math.round(r() * 200),
      "Memo/Description": `[Rippling] Contractor Payments - Amount for ${nm}. Payroll run: ${MON[st.getMonth()]} ${String(st.getDate()).padStart(2, "0")} - ${MON[en.getMonth()]} ${String(en.getDate()).padStart(2, "0")}:  Homes - Hourly Int'l Contractors` }); } });
  return { opps_created, opps_closed, pipeline, appointments, leads, calls, directory, targets, uw_cycle, uw_accuracy, mkt_spend, mkt_vendor_map, mkt_roster, mkt_labor };
}
...
const KPIS = {
  ...
  // Revives = Dead → New Lead only (Nurture / Contact Attempted → New Lead are status recycles, not revives) — same rule as blended CPL.
  reactivated_leads: { id: "reactivated_leads", label: "Reactivated Leads", dataset: "mkt_revives", format: "number", domain: "marketing",
    compute: (rows) => new Set(rows.filter((r) => /^dead$/i.test(String(r.oldValue ?? "").trim())).map((r) => r.leadId)).size,
    subStat: () => "Dead → New Lead · unique leads" },
...
// Underwriting view. Two lenses, each tagged: cycle time (Appt Set -> UW Complete, windowed by appt-set date)
...
function UnderwritingView({ store, range, dir, org }) {
  ...
  // ── Cycle time (Appt Set → UW Complete) · windowed by appt-set date (deals UW was requested on) ──────
  const cycleRows = useMemo(() => applyFilters(store.uw_cycle || [], DATASETS.uw_cycle, org, range, dir), [store, org, range, dir]);
  const cycle = useMemo(() => {
    const durs = [], perU = {}, perDD = {}, monthAgg = {};
    cycleRows.forEach((r) => {
      ...
      const dd = s(r.dd); if (dd) (perDD[dd] = perDD[dd] || []).push(sec);
      const mk = monthKey(r.apptSet); if (mk) (monthAgg[mk] = monthAgg[mk] || []).push(sec);
    });
    ...
    const rollup = (m) => Object.entries(m).map(([name, v]) => ({ name, n: v.length, med: median(v) })).sort((x, y) => y.n - x.n);
    const trend = Object.keys(monthAgg).sort().map((k) => ({ label: k.slice(2), value: Math.round(median(monthAgg[k]) / 3600 * 10) / 10 }));
    return { n: durs.length, median: median(durs), p90: quantile(durs, 0.9), avg: mean(durs), buckets, perU: rollup(perU), perDD: rollup(perDD), trend };
  }, [cycleRows]);
  // "Awaiting UW" is a live QUEUE, not a windowed metric — it must NOT ride the date filter (an appt set months
  // ago and still not underwritten is in the queue today). Rep/Team scope still applies; the date range does not.
  const backlog = useMemo(() => {
    const rows = applyFilters(store.uw_cycle || [], DATASETS.uw_cycle, org, null, dir);
    return rows.filter((r) => { const a = parseDateTime(r.apptSet), b = parseDateTime(r.uwComplete); return a && !b; }).length;
  }, [store, org, dir]);
  ...
  const acc = useMemo(() => {
    ...
    const scored = []; let closeMatched = 0, flipNoArv = 0, inWindow = 0, skippedNoBasis = 0; const fees = []; const moAgg = {};
    accRows.forEach((r) => {
      // Date gate FIRST — the cohort is "deals that closed in this window" (undated only under All-time).
      const c = closeMap.get(s(r.oid)); if (c) closeMatched++;
      const dated = c ? inRange(c.closeDate) : !range; if (!dated) return;
      inWindow++;
      const fee = c ? num(c.revenue) : 0; if (fee > 0) fees.push(fee); // fee/revenue per closed deal, from the join
      const cm = c ? monthKey(c.closeDate) : null; if (cm) (moAgg[cm] || (moAgg[cm] = { n: 0, deltas: [] })).n++; // deals closed / month
      const bucket = bucketOf(r.recordType), sold = num(r.endBuyer);
      let basis = null;
      if (bucket === "Back-End") {
        // basis = front-end opp's ARV. Prefer it pulled directly onto the row (stable, no matching); else match
        // the front-end by name from any ARV-bearing row; else the back-end's own ARV; else skip.
        const direct = num(r.feArv), byName = num(arvByName.get(norm(r.feName))), own = num(r.arv);
        basis = direct > 0 ? direct : (byName > 0 ? byName : (own > 0 ? own : null));
        if (!(basis > 0)) flipNoArv++;
      }
      else { const a1 = num(r.asIs), a2 = num(r.asIsUw); basis = a1 > 0 ? a1 : (a2 > 0 ? a2 : null); }
      if (!(basis > 0 && sold > 0)) { skippedNoBasis++; return; } // in-window but missing a valuation basis or sold price
      const delta = (sold - basis) > 0 ? (sold - basis) / sold : (basis - sold) / basis;
      const signed = (sold - basis) / basis;
      if (cm) moAgg[cm].deltas.push(delta);
      scored.push({ ...r, bucket, basis, sold, delta, signed, over: sold >= basis, closeM: cm,
    ...
    const rollup = (field) => { const m = {}; scored.forEach((x) => { const k = s(x[field]); if (!k) return; (m[k] = m[k] || []).push(x.delta); });
      return Object.entries(m).map(([name, v]) => ({ name, n: v.length, avg: mean(v) })).sort((a, b) => a.avg - b.avg); };
    const perURows = rollup("underwriter"), perDDRows = rollup("dd");
    // trend: avg |delta| (left axis) AND deals closed (right axis) per close month, for the dual-axis chart
    const trend = Object.keys(moAgg).sort().map((k) => ({ label: k.slice(2),
      value: moAgg[k].deltas.length ? Math.round(mean(moAgg[k].deltas) * 1000) / 10 : 0, count: moAgg[k].n }));
    ...
    return { total: accRows.length, inWindow, skippedNoBasis, scored: scored.length, closeMatched, flipNoArv, avg: mean(deltas), med: median(deltas),
      avgFee: fees.length ? mean(fees) : null, medFee: median(fees), totalFee: fees.reduce((a, b) => a + b, 0), feeN: fees.length,
      within10: within(0.10), within5: within(0.05), over, under,
      overAvg: over.length ? mean(over.map((x) => x.delta)) : null, underAvg: under.length ? mean(under.map((x) => x.delta)) : null,
      shareUnder: scored.length ? under.length / scored.length : 0,
      byBucket, perU: perURows, perDD: perDDRows, trend, avgSpread: spreads.length ? mean(spreads) : null, avgDiscount: discounts.length ? mean(discounts) : null,
  ...
  return (
    <div className="flex flex-col gap-5" id="underwriting-view">
      <div className="rounded-2xl p-3.5 text-[12px] leading-relaxed" style={{ background: T.track, color: T.sub }}>
        <b style={{ color: T.ink }}>Underwriting</b> tracks two things. <b>Cycle time</b> <span className="inline-block mx-1"><DispoTag kind="dated" /></span> = how long from appointment set to UW complete; the window shows the deals <b>UW was requested on</b> (by appt-set date). <b>Accuracy</b> <span className="inline-block mx-1"><DispoTag kind="dated" /></span> = the valuation delta on closed deals — <i>(sold − as-is) ÷ sold</i> when sold beats as-is, else <i>(as-is − sold) ÷ as-is</i>; always positive, <b>lower is better</b>. Flips (Back-End) compare the front-end opp's ARV to the back-end sold price. Accuracy's window shows the deals that <b>closed</b> in it (by close date); a closed deal with no matched close date only appears under the All-time window.
      </div>

      {/* CYCLE TIME */}
      <Panel title="Underwriting cycle time — appointment set → UW complete">
        <div className="flex items-start gap-2 flex-wrap mb-3"><DispoTag kind="dated" /><span className="text-[11px]" style={{ color: T.faint }}>by appt-set date · deals UW was requested on · {spanLabel}</span></div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <UwStat label="UW Completed" value={cycle.n.toLocaleString()} sub="requests in window now complete" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
          <UwStat label="Awaiting UW" value={backlog.toLocaleString()} sub="appt set, not yet complete · all open" tag="snapshot" />
        ...
        {cycle.perDD.length > 0 && (<div className="mt-5"><div className="text-[11px] uppercase tracking-wide mb-2" style={{ color: T.faint }}>By due diligence specialist</div>
          <UwTable cols={[{ key: "name", label: "DD specialist" }, { key: "n", label: "Completed", fmt: (v) => v.toLocaleString() }, { key: "med", label: "Median cycle", fmt: (v) => fmtSpan(v) }]} rows={cycle.perDD} /></div>)}
      ...
      <Panel title="Underwriting accuracy — valuation delta on closed deals">
        <div className="flex items-start gap-2 flex-wrap mb-3"><DispoTag kind="dated" /><span className="text-[11px]" style={{ color: T.faint }}>by close date · deals that closed in {spanLabel}</span></div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <UwStat label="Deals scored" value={acc.scored.toLocaleString()} sub={`${acc.inWindow} closed in window · ${acc.skippedNoBasis} skipped (no basis/price)`} />
          <UwStat label="Avg |delta|" value={pctF(acc.avg)} sub="absolute · lower is better" tone={acc.avg <= 0.1 ? T.good : acc.avg <= 0.2 ? T.warn : T.bad} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
          <UwStat label="Δ · sold over as-is" value={pctF(acc.overAvg)} sub={`${acc.over.length} deals · UW under-valued`} />
          <UwStat label="Δ · sold under as-is" value={pctF(acc.underAvg)} sub={`${acc.under.length} deals · UW over-valued`} />
          <UwStat label="Sold under as-is" value={pctF(acc.shareUnder)} sub={acc.shareUnder >= 0.5 ? "most deals — UW runs high" : "minority — UW runs low"} tone={acc.shareUnder >= 0.6 ? T.warn : T.ink} />
        </div>
        <div className="text-[11px] mt-2" style={{ color: T.faint }}>All three deltas are absolute magnitudes (over- and under-valuations never cancel): the headline Avg |delta| is every deal's absolute miss; these split it into deals that sold above vs below the as-is.</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
          <UwStat label="Avg fee size" value={money(acc.avgFee)} sub={`revenue per closed deal · ${acc.feN} of ${acc.inWindow}`} />
        ...
        {acc.perDD.length > 0 && (<div className="mt-5"><div className="text-[11px] uppercase tracking-wide mb-2" style={{ color: T.faint }}>By due diligence specialist (best accuracy first)</div>
          <UwTable cols={[{ key: "name", label: "DD specialist" }, { key: "n", label: "Deals", fmt: (v) => v.toLocaleString() }, { key: "avg", label: "Avg |delta|", fmt: (v) => pctF(v) }]} rows={acc.perDD} /></div>)}
        {acc.trend.length > 1 && (<div className="mt-5"><div className="text-[11px] uppercase tracking-wide mb-2" style={{ color: T.faint }}>Avg |delta| &amp; deals closed by month</div>
          <div className="flex items-center gap-4 mb-2 text-[11px]" style={{ color: T.sub }}>
            <span className="inline-flex items-center gap-1.5"><span style={{ width: 10, height: 10, borderRadius: 2, background: T.accent, display: "inline-block" }} />Avg |delta| % (left)</span>
            <span className="inline-flex items-center gap-1.5"><span style={{ width: 10, height: 10, borderRadius: 2, background: T.sub, display: "inline-block" }} />Deals closed (right)</span>
          </div>
          <div style={{ height: 220 }}><ResponsiveContainer width="100%" height="100%">
            <BarChart data={acc.trend} margin={{ top: 5, right: 6, left: 0, bottom: 0 }} barGap={2}>
              <CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false} />
              <XAxis dataKey="label" tick={{ fontSize: 11, fill: T.faint }} axisLine={{ stroke: T.border }} tickLine={false} />
              <YAxis yAxisId="left" tick={{ fontSize: 11, fill: T.faint }} axisLine={false} tickLine={false} />
              <YAxis yAxisId="right" orientation="right" allowDecimals={false} tick={{ fontSize: 11, fill: T.faint }} axisLine={false} tickLine={false} />
              <Tooltip cursor={{ fill: T.track }} contentStyle={UW_TREND_TIP} formatter={(v, n) => [String(n).includes("delta") ? `${v}%` : v, n]} />
              <Bar yAxisId="left" dataKey="value" name="Avg |delta| %" fill={T.accent} radius={[4, 4, 0, 0]} />
              <Bar yAxisId="right" dataKey="count" name="Deals closed" fill={T.sub} radius={[4, 4, 0, 0]} />
            </BarChart></ResponsiveContainer></div></div>)}
        <div className="text-[11px] mt-3" style={{ color: T.faint }}>Basis = the first "As Is Valuation" column (falls back to the second when blank). Back-End flips use the front-end opp's ARV — pulled directly from a "Front-End Opportunity: ARV" column if present, otherwise matched by name{acc.flipNoArv > 0 ? ` (${acc.flipNoArv} flip${acc.flipNoArv === 1 ? "" : "s"} skipped — no front-end ARV found)` : ""}. {acc.closeMatched} of {acc.total} rows have a matched close date; deals without one appear only under the All-time window.</div>
      </Panel>
...
// Shared appointment show-rate + type-group helpers (used by both the VP and AM/FU metric bundles).
const apptSrOf = (rows) => { const den = rows.filter((r) => !apptExcluded(r.outcome)); const met = den.filter((r) => apptAttended(r.outcome)).length; return { sched: den.length, met, rate: den.length ? met / den.length : null }; };
const apptGroupOf = (rows) => ({
  total: rows.length,
  countByType: VP_APPT_TYPES.map((t) => ({ ...t, value: rows.filter((r) => vpApptType(r) === t.key).length })),
  showByType: VP_APPT_TYPES.map((t) => ({ ...t, ...apptSrOf(rows.filter((r) => vpApptType(r) === t.key)) })),
  showOverall: apptSrOf(rows),
});
// Like apptGroupOf but with a split date axis: set COUNTS come from countRows (Created-Date population),
// while SHOW RATE comes from showRows (Start-Date population). Used by the AM/FU card so "appts set" counts
// by when they were booked, and show rate by whether the meeting (Start) fell in the window.
const apptGroupMixed = (countRows, showRows) => ({
  total: countRows.length,
  countByType: VP_APPT_TYPES.map((t) => ({ ...t, value: countRows.filter((r) => vpApptType(r) === t.key).length })),
  showByType: VP_APPT_TYPES.map((t) => ({ ...t, ...apptSrOf(showRows.filter((r) => vpApptType(r) === t.key)) })),
  showOverall: apptSrOf(showRows),
});
...
function vpMetricsFor(store, dir, org, range, rangeFwd) {
  ...
  const selfGroup = apptGroupOf(selfSetAppts);                 // #3 Self-set appointments
  const assignedGroup = apptGroupOf(appts.filter((r) => !isSelf(r))); // #4 Assigned (by others) appointments
  // #5 ARIP → Deal Review %.
  ...
  const aripToDR = { cohort, adv, rate: cohort ? adv / cohort : null };
  // #6 Contracts Sent, #8 Pipeline forecast, #9 Closed revenue.
  const contracts = computeKpi(KPIS.contracts_sent, store, dir, org, range, range).value || 0;
  const pipeline = computeKpi(KPIS.pipeline_forecast, store, dir, org, rangeFwd, rangeFwd).value || 0;
  const closedRev = computeKpi(KPIS.closed_revenue, store, dir, org, range, range).value || 0;
  // #7 VP's own OUTBOUND calls.
  ...
  // Extra headline KPIs (the ones shown as tiles) so the Per-VP breakout can list every metric.
  const kv = (id, fwd) => { const r = fwd ? rangeFwd : range; return computeKpi(KPIS[id], store, dir, org, r, r).value; };
  const extra = { dealsOutArip: kv("arip_dealreview"), revOutArip: kv("rev_out_of_arip"), avgDeal: kv("avg_deal"),
    dealsClosed: kv("deals_closed"), oppsAssigned: kv("opps_assigned"), oppsCreated: kv("opps_created"),
    oppsDeaded: kv("opps_deaded"), avgIcp: kv("avg_icp_per_appt") };
  return { arips, assigned, attByType, attRouting, selfGroup, assignedGroup, aripToDR, contracts, pipeline, closedRev, calls, extra };
}

// AM / FU metric bundle — mirrors vpMetricsFor from the setter/opp-tag perspective (JP spec): appointments are
// the ones the rep SET (Created By ∈ scope, excl. Listing-Partner-routed), and ARIP / revenue / deals credit the
// rep wherever they're tagged on the opp (Acquisition Manager 1/2 or Follow-Up) — the same OR-match applyFilters
// already uses via repFields. Appt-type + show-rate machinery is shared with VP (apptGroupOf / vpApptType).
function amFuMetricsFor(store, dir, org, range, rangeFwd) {
  const setAppts = applyFilters(store.appointments_attended || [], DATASETS.appointments_attended, org, range, dir).filter((r) => !r.lpAssigned); // by Start (show rate + attended)
  const setCreated = applyFilters(store.appointments || [], DATASETS.appointments, org, range, dir).filter((r) => !r.lpAssigned); // by Created Date (set counts)
  const attended = setAppts.filter((r) => apptAttended(r.outcome));
  const setGroup = apptGroupMixed(setCreated, setAppts);     // counts by Created Date, show rate by Start
  const arips = applyFilters(store.arip_entered || [], DATASETS.arip_entered, org, range, dir).length; // opps where rep is AM1/AM2/FUS entering ARIP (window count)
  const attByType = {                                        // attended (met) → ARIP: IP alone, Virtual+FU combined, overall
    ip: attended.filter((r) => vpApptType(r) === "ip").length,
    virtfu: attended.filter((r) => { const k = vpApptType(r); return k === "virtual" || k === "fu"; }).length,
    overall: attended.length,
  };
  // ARIP → Deal Review %: numerator = the rep's ARIP→Deal-Review transitions from the AM/FU-attributed
  // "Arips to Deal Review" report (New Value = Deal Review, out-of-ARIP flag = 1) — the SAME source the Rep
  // scorecard uses, so it carries Acquisition-Manager AND Follow-Up attribution (stage_history, in the
  // transactions workbook, does not populate the AM/FU fields, so a cohort keyed off it comes back empty).
  // Denominator = opps entering ARIP in the window (arips, above). A window ratio, so it can exceed 100%.
  const aripScope = repsInScope(dir, org);
  const aripInScope = (r) => { if (!aripScope) return true; return [r.rep, r.followUp].some((v) => aripScope.has(String(v ?? "").trim())); };
  const aripInWin = (d) => { if (!range) return true; const t = parseDate(d); return !!(t && t >= range.start && t <= range.end); };
  const drAdv = (store.arip || []).filter((r) => aripInScope(r) && aripInWin(r.date) && String(r.newValue).trim() === "Deal Review" && Number(r.outArip) === 1).length;
  const aripToDR = { cohort: arips, adv: drAdv, rate: arips ? drAdv / arips : null };
  const pipeline = computeKpi(KPIS.pipeline_forecast, store, dir, org, rangeFwd, rangeFwd).value || 0;
  const closedRev = computeKpi(KPIS.closed_revenue, store, dir, org, range, range).value || 0;
  const callRows = applyFilters(store.calls || [], DATASETS.calls, org, range, dir); // every logged call (AM/FU count all calls, matching the Calls Logged tile)
  const cmins = callRows.reduce((s, r) => s + num(r.durationMin), 0);
  const calls = { calls: callRows.length, minutes: cmins, qcs: callRows.filter(isQC).length, avgSec: callRows.length ? (cmins / callRows.length) * 60 : 0 };
  const outRows = callRows.filter((r) => /outgoing/i.test(String(r.subject ?? ""))); // outbound subset
  const outMin = outRows.reduce((s, r) => s + num(r.durationMin), 0);
  const outbound = { calls: outRows.length, minutes: outMin, avgSec: outRows.length ? (outMin / outRows.length) * 60 : 0 };
  const kv = (id, fwd) => { const r = fwd ? rangeFwd : range; return computeKpi(KPIS[id], store, dir, org, r, r).value; };
  const extra = { dealsOutArip: kv("arip_dealreview"), revOutArip: kv("rev_out_of_arip"), avgDeal: kv("avg_deal"),
    dealsClosed: kv("deals_closed"), oppsCreated: kv("opps_created"), leadsClaimed: kv("leads_claimed"),
    leadsDeaded: kv("leads_deaded"), avgIcp: kv("avg_icp_per_appt") };
  return { arips, setGroup, attByType, aripToDR, pipeline, closedRev, calls, outbound, extra };
}
...
function VpStat({ label, value, sub, tone }) {
  ...
  return (<div className="lh-card rounded-2xl p-4 flex flex-col gap-2 h-full" style={{ background: T.card, border: `1px solid ${T.border}`, boxShadow: T.shadow }}>
    <div className="text-[11px] font-medium" style={{ color: T.faint }}>{label}</div>
    <div className="text-[23px] font-bold leading-none tracking-tight" style={{ color: c, fontVariantNumeric: "tabular-nums" }}>{value}</div>
    {sub && <div className="text-[11px] leading-snug" style={{ color: T.sub }}>{sub}</div>}
  </div>);
}
// Larger "hero" tile — used to pull the headline revenue numbers out above the metric grid.
function VpHeroStat({ label, value, sub, tone }) {
  const c = tone === "good" ? T.good : tone === "bad" ? T.bad : T.ink;
  return (<div className="lh-card rounded-2xl p-5 flex flex-col gap-2 h-full" style={{ background: T.card, border: `1px solid ${T.border}`, boxShadow: T.shadow }}>
    <div className="text-[12px] font-semibold uppercase" style={{ color: T.faint, letterSpacing: "0.04em" }}>{label}</div>
    <div className="text-[38px] font-bold leading-none tracking-tight" style={{ color: c, fontVariantNumeric: "tabular-nums" }}>{value}</div>
    {sub && <div className="text-[12px] leading-snug" style={{ color: T.sub }}>{sub}</div>}
  </div>);
}
...
function VpPerVpTable({ perVp, groups }) {
  const G = groups || [
    { g: "Funnel & ARIP", rows: [
      { h: "Opps Assigned", get: (m) => (m.extra.oppsAssigned ?? 0).toLocaleString() },
      { h: "Opps → ARIP", get: (m) => m.arips.toLocaleString() },
      { h: "Deals Out of ARIP", get: (m) => (m.extra.dealsOutArip ?? 0).toLocaleString() },
    ...
    { g: "Activity", rows: [
      ...
      { h: "Opps Created", get: (m) => (m.extra.oppsCreated ?? 0).toLocaleString() },
      { h: "Opps Deaded", get: (m) => (m.extra.oppsDeaded ?? 0).toLocaleString() },
      { h: "Avg ICP · Appt Set", get: (m) => fmt(m.extra.avgIcp, "decimal") },
    ] },
    { g: "Revenue", rows: [
      { h: "Pipeline (forecast)", get: (m) => fmt(m.pipeline, "currency") },
      { h: "Revenue Out of ARIP", get: (m) => fmt(m.extra.revOutArip, "currency") },
      { h: "Deals Closed", get: (m) => (m.extra.dealsClosed ?? 0).toLocaleString() },
      { h: "Closed revenue", get: (m) => fmt(m.closedRev, "currency") },
      { h: "Avg Deal Size", get: (m) => fmt(m.extra.avgDeal, "currency") },
  ...
  const cell = { borderBottom: `1px solid ${T.border2}` };
  const METRIC_W = 200, COL_W = 130;
  // Frozen metric column (pinned on horizontal scroll) + frozen header row (pinned on vertical scroll).
  // Pinned cells are opaque (T.card) so scrolling values slide under them instead of bleeding through.
  const froze = { position: "sticky", left: 0, zIndex: 2, background: T.card, borderRight: `1px solid ${T.border2}` };
  return (<div className="lh-frozen">
    <table className="text-[13px]" style={{ borderCollapse: "separate", borderSpacing: 0, tableLayout: "fixed", width: "100%", minWidth: METRIC_W + nVp * COL_W }}>
      <colgroup><col style={{ width: METRIC_W }} />{perVp.map(({ vp }) => <col key={vp} />)}</colgroup>
      <thead><tr>
        <th className="py-2.5 px-3.5 text-left text-[11px] font-medium" style={{ ...cell, ...froze, top: 0, zIndex: 5, color: T.faint }}>Metric</th>
        {perVp.map(({ vp }) => <th key={vp} className="py-2.5 px-3.5 text-right whitespace-nowrap text-[13px]" style={{ ...cell, position: "sticky", top: 0, zIndex: 3, background: T.card, color: T.ink, fontWeight: 600 }}>{vp}</th>)}
      </tr></thead>
      <tbody>
        {G.map((group) => (
          <React.Fragment key={group.g}>
            <tr>
              <td className="pt-4 pb-1.5 px-3.5 text-[11px] font-semibold" style={{ ...froze, color: T.accent }}>{group.g}</td>
              <td colSpan={nVp} style={{ background: T.card }} />
            </tr>
            {group.rows.map((r, ri) => (
              <tr key={r.h} className="lh-row" style={{ color: T.ink, background: ri % 2 ? T.zebra : "transparent" }}>
                <td className="py-2.5 px-3.5" title={r.h} style={{ ...cell, ...froze, color: T.ink, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.h}</td>
                {perVp.map(({ vp, m }) => (
...
function VpFocus({ store, dir, org, range, rangeFwd, drillLabel, results }) {
  ...
  const rv = (id) => { const r = results && results[id]; return r && r.value != null ? fmt(r.value, KPIS[id].format) : "—"; };
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-2.5">
        <SubHead label="Results" note="ARIP flow, pipeline & revenue" />
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          <VpStat label="Opps → ARIP" value={b.arips.toLocaleString()} sub="reached ARIP" />
          <VpStat label="Deals Out of ARIP" value={rv("arip_dealreview")} sub="advanced past ARIP" />
          <VpStat label="ARIP → Deal Review" value={vpPct(b.aripToDR.rate)} sub={b.aripToDR.cohort ? `${b.aripToDR.adv}/${b.aripToDR.cohort} advanced` : "no ARIP cohort"} tone="good" />
          <VpStat label="Revenue Out of ARIP" value={rv("rev_out_of_arip")} />
          <VpStat label="Pipeline (forecast)" value={fmt(b.pipeline, "currency")} />
          <VpStat label="Avg Deal Size" value={rv("avg_deal")} />
          <VpStat label="Deals Closed" value={rv("deals_closed")} tone="good" />
          <VpStat label="Closed Revenue" value={fmt(b.closedRev, "currency")} tone="good" />
        </div>
        <div className="text-[11px] px-0.5 leading-snug" style={{ color: T.faint }}><b style={{ color: T.sub }}>Opps → ARIP</b> = opps that reached ARIP · <b style={{ color: T.sub }}>Deals Out of ARIP</b> = opps that advanced past ARIP · <b style={{ color: T.sub }}>ARIP → Deal Review</b> = the % reaching Deal Review (measured on its own cohort, so it won't equal the two counts divided).</div>
      </div>

      <div className="flex flex-col gap-2.5">
        <SubHead label="Activity & other" note="assignments, calls, contracts & lead flow" />
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          <VpStat label="Opps Assigned" value={rv("opps_assigned")} />
          <VpStat label="Contracts Sent" value={b.contracts.toLocaleString()} />
          <VpStat label="Talk Time" value={fmt(b.calls.minutes, "minutes")} />
          <VpStat label="Outbound Calls" value={b.calls.calls.toLocaleString()} sub={`avg ${fmtDur(b.calls.avgSec)}`} />
          <VpStat label="QCs" value={b.calls.qcs.toLocaleString()} />
          <VpStat label="Opps Created" value={rv("opps_created")} />
          <VpStat label="Opps Deaded" value={rv("opps_deaded")} />
          <VpStat label="Avg ICP · Appt Set" value={rv("avg_icp_per_appt")} />
        </div>
      </div>

      <div className="flex items-center gap-2.5 pt-1" style={{ breakBefore: "page", pageBreakBefore: "always" }}>
        <div className="w-1 h-6 rounded-sm" style={{ background: T.accent }} />
        <div className="flex-1 min-w-0">
          <div className="text-[14px] font-bold" style={{ color: T.ink, letterSpacing: "-0.01em" }}>VP Focus · <span style={{ color: T.accent }}>{drillLabel}</span></div>
          <div className="text-[11px]" style={{ color: T.faint }}>Appointment funnel &amp; appointments{vps.length > 1 ? ` · ${vps.length} VPs blended (per-VP below)` : ""} · appts by Start · ARIP = the VP's opps entering ARIP in the same window</div>
        </div>
      </div>
...
// Per-rep breakout groups for the AM / FU view (consumed by VpPerVpTable via its `groups` prop).
const amfuTypeVal = (m, key) => { const t = m.setGroup.countByType.find((x) => x.key === key); return t ? t.value : 0; };
const amfuTypeRate = (m, key) => { const t = m.setGroup.showByType.find((x) => x.key === key); return t ? t.rate : null; };
const AMFU_GROUPS = [
  { g: "Funnel & ARIP", rows: [
    { h: "Opps → ARIP", get: (m) => m.arips.toLocaleString() },
    { h: "Deals Out of ARIP", get: (m) => (m.extra.dealsOutArip ?? 0).toLocaleString() },
    { h: "Appts set → ARIP", get: (m) => ratio1inN(m.arips, m.setGroup.total), sub: (m) => `${m.setGroup.total} set → ${m.arips}` },
    { h: "Appts attended → ARIP", get: (m) => ratio1inN(m.arips, m.attByType.overall), sub: (m) => `${m.attByType.overall} met → ${m.arips}` },
    { h: "ARIP → Deal Review", get: (m) => vpPct(m.aripToDR.rate) },
  ] },
  { g: "Appointments", rows: [
    { h: "In Person set", get: (m) => amfuTypeVal(m, "ip").toLocaleString() },
    { h: "Virtual set", get: (m) => amfuTypeVal(m, "virtual").toLocaleString() },
    { h: "Follow Up set", get: (m) => amfuTypeVal(m, "fu").toLocaleString() },
    { h: "Appts set", get: (m) => m.setGroup.total.toLocaleString() },
    { h: "Show rate · blended", get: (m) => vpPct(m.setGroup.showOverall.rate) },
    { h: "Show rate · IP", get: (m) => vpPct(amfuTypeRate(m, "ip")) },
    { h: "Show rate · Virtual", get: (m) => vpPct(amfuTypeRate(m, "virtual")) },
    { h: "Show rate · Follow Up", get: (m) => vpPct(amfuTypeRate(m, "fu")) },
  ] },
  { g: "Activity", rows: [
    { h: "Calls logged", get: (m) => m.calls.calls.toLocaleString() },
    { h: "Outbound calls", get: (m) => m.outbound.calls.toLocaleString() },
    { h: "Talk Time", get: (m) => fmt(m.calls.minutes, "minutes") },
    { h: "Avg call (all)", get: (m) => fmtDur(m.calls.avgSec) },
    { h: "Avg outbound", get: (m) => fmtDur(m.outbound.avgSec) },
    { h: "QCs", get: (m) => m.calls.qcs.toLocaleString() },
    { h: "Opps Created", get: (m) => (m.extra.oppsCreated ?? 0).toLocaleString() },
    { h: "Leads Claimed", get: (m) => (m.extra.leadsClaimed ?? 0).toLocaleString() },
    { h: "Leads Deaded", get: (m) => (m.extra.leadsDeaded ?? 0).toLocaleString() },
    { h: "Avg ICP · Appt Set", get: (m) => fmt(m.extra.avgIcp, "decimal") },
  ] },
  { g: "Revenue", rows: [
    { h: "Pipeline (forecast)", get: (m) => fmt(m.pipeline, "currency") },
...
// ── AM / FU FOCUS (drilldown) ──────────────────────────────────────────────────────────────────────
// Consolidated section at the top of Sales whenever scope is the AM team, the FU team, the combined
// AMs+Follow-Up union, or a single AM/FU rep. Mirrors VP Focus but from the setter/opp-tag perspective:
// appts the rep SET, and ARIP/revenue/deals credited wherever the rep is tagged (AM1/AM2/FUS).
function AmFuFocus({ store, dir, org, range, rangeFwd, drillLabel, results }) {
  const reps = useMemo(() => [...(repsInScope(dir, org) || new Set())].sort(), [dir, org]);
  const b = useMemo(() => amFuMetricsFor(store, dir, org, range, rangeFwd), [store, dir, org, range, rangeFwd]);
  const rv = (id) => { const r = results && results[id]; return r && r.value != null ? fmt(r.value, KPIS[id].format) : "—"; };
  const roleOf = (rep) => String(dir.byRep[rep]?.role || "");
  const amReps = useMemo(() => reps.filter((r) => /acqu/i.test(roleOf(r))), [reps, dir]);
  const fuReps = useMemo(() => reps.filter((r) => /follow.?up/i.test(roleOf(r))), [reps, dir]);
  const allReps = useMemo(() => [...amReps, ...fuReps], [amReps, fuReps]); // AMs first, then Follow-Ups
  const perRep = useMemo(() => (allReps.length > 1 ? allReps.map((rep) => ({ vp: rep, m: amFuMetricsFor(store, dir, { ...ALL_ORG, rep }, range, rangeFwd) })) : []), [store, dir, allReps, range, rangeFwd]);
  const allAM = reps.length > 0 && reps.every((r) => /acqu/i.test(roleOf(r)));
  const allFU = reps.length > 0 && reps.every((r) => /follow.?up/i.test(roleOf(r)));
  const focusLabel = allFU ? "Follow-Up Focus" : allAM ? "AM Focus" : "AM / FU Focus";
  const ipN = b.attByType.ip, vfuN = b.attByType.virtfu, totN = b.attByType.overall;
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-2.5">
        <SubHead label="Revenue" note="pipeline forecast & closed — credited wherever the rep is tagged on the opp" />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <VpHeroStat label="Pipeline (forecast)" value={fmt(b.pipeline, "currency")} sub="open opps · projected revenue" />
          <VpHeroStat label="Closed Revenue" value={fmt(b.closedRev, "currency")} sub="deals closed in the period" tone="good" />
        </div>
      </div>

      <div className="flex flex-col gap-2.5">
        <SubHead label="Results" note="ARIP flow & deal output" />
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <VpStat label="Opps → ARIP" value={rv("opps_to_arip")} sub="reached ARIP" />
          <VpStat label="Deals Out of ARIP" value={rv("arip_dealreview")} sub="advanced past ARIP" />
          <VpStat label="ARIP → Deal Review" value={vpPct(b.aripToDR.rate)} sub={b.aripToDR.cohort ? `${b.aripToDR.adv}/${b.aripToDR.cohort} reached DR` : "no ARIP cohort"} tone="good" />
          <VpStat label="Revenue Out of ARIP" value={rv("rev_out_of_arip")} />
          <VpStat label="Avg Deal Size" value={rv("avg_deal")} />
          <VpStat label="Deals Closed" value={rv("deals_closed")} tone="good" />
        </div>
        <div className="text-[11px] px-0.5 leading-snug" style={{ color: T.faint }}>ARIP, revenue &amp; deals credit this rep wherever they're tagged on the opp — <b style={{ color: T.sub }}>AM 1, AM 2, or Follow-Up</b>. <b style={{ color: T.sub }}>Opps → ARIP</b> = the rep's opps that reached ARIP · <b style={{ color: T.sub }}>ARIP → Deal Review</b> = the % of ARIP opps reaching Deal Review (a window ratio, so it can top 100%).</div>
      </div>

      <div className="flex flex-col gap-2.5">
        <SubHead label="Call activity" note="dials, talk time & quality conversations" />
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <VpStat label="Outbound Calls" value={b.outbound.calls.toLocaleString()} sub={`avg ${fmtDur(b.outbound.avgSec)}`} />
          <VpStat label="Calls Logged" value={b.calls.calls.toLocaleString()} sub={`all calls · avg ${fmtDur(b.calls.avgSec)}`} />
          <VpStat label="Talk Time" value={fmt(b.calls.minutes, "minutes")} />
          <VpStat label="QCs" value={b.calls.qcs.toLocaleString()} />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2.5">
        <SubHead label="Activity & other" note="appointments, opps & lead flow" />
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <VpStat label="Appointments Set" value={rv("appointments")} />
          <VpStat label="Opps Created" value={rv("opps_created")} />
          <VpStat label="Leads Claimed" value={rv("leads_claimed")} />
          <VpStat label="Leads Deaded" value={rv("leads_deaded")} />
          <VpStat label="Avg ICP · Appt Set" value={rv("avg_icp_per_appt")} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2.5 pt-1" style={{ breakBefore: "page", pageBreakBefore: "always" }}>
        <div className="w-1 h-6 rounded-sm" style={{ background: T.accent }} />
        <div className="flex-1 min-w-0">
          <div className="text-[14px] font-bold" style={{ color: T.ink, letterSpacing: "-0.01em" }}>{focusLabel} · <span style={{ color: T.accent }}>{drillLabel}</span></div>
          <div className="text-[11px]" style={{ color: T.faint }}>Appointments the rep set &amp; their conversion{reps.length > 1 ? ` · ${reps.length} reps blended (per-rep below)` : ""} · set counts by Created Date, show rate & attended→ARIP by Start · ARIP = the rep's opps entering ARIP in the same window</div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <VpCard n="1" title="Appts attended → ARIP" hint="met appointments the rep set · ratio = appts ÷ ARIPs in window">
          <VpRatio arips={b.arips} appts={totN} unit="met" />
          <VpGroup label="By type" rows={[
            { label: "In Person", value: ipN, wide: true, right: ipN ? `${ratio1inN(b.arips, ipN)} · ${Math.round((b.arips / ipN) * 100)}%` : "—" },
            { label: "Virtual + Follow Up", value: vfuN, wide: true, right: vfuN ? `${ratio1inN(b.arips, vfuN)} · ${Math.round((b.arips / vfuN) * 100)}%` : "—" },
          ]} />
        </VpCard>
        <VpApptGroupCard n="2" title="Appointments set" hint="set counts by appointment Created Date Â· show rate by Start date" g={b.setGroup} />
      </div>

      {perRep.length > 0 && (() => {
        const rows = [...perRep].sort((a, b2) => (b2.m.aripToDR.rate ?? -1) - (a.m.aripToDR.rate ?? -1));
        const maxR = Math.max(0.001, ...rows.map((x) => x.m.aripToDR.rate || 0));
        return (
          <VpCard title="ARIP → Deal Review by rep" hint="opps reaching Deal Review ÷ opps entering ARIP · window ratio · credited by AM / Follow-Up tag">
            <div className="flex flex-col gap-2">
              {rows.map(({ vp, m }) => (
                <div key={vp} className="flex items-center gap-3">
                  <span className="text-[13px] whitespace-nowrap" style={{ color: T.sub, width: 150, overflow: "hidden", textOverflow: "ellipsis" }}>{vp}</span>
                  <span className="flex-1 h-2 rounded-full" style={{ background: T.track }}>
                    <span className="block h-2 rounded-full" style={{ width: `${Math.min(100, ((m.aripToDR.rate || 0) / maxR) * 100)}%`, background: T.accent }} />
                  </span>
                  <span className="text-[10px] tabular-nums whitespace-nowrap" style={{ color: T.faint, width: 48, textAlign: "right" }}>{m.aripToDR.cohort ? `${m.aripToDR.adv}/${m.aripToDR.cohort}` : "—"}</span>
                  <span className="text-[13px] font-semibold tabular-nums whitespace-nowrap" style={{ color: T.ink, width: 54, textAlign: "right" }}>{vpPct(m.aripToDR.rate)}</span>
                </div>))}
            </div>
          </VpCard>);
      })()}
      ...
      {perRep.length > 0 && (
        <VpCard title={allAM ? "Per-AM breakout" : allFU ? "Per-FU breakout" : "Per-rep breakout"} hint={allAM ? "every metric, by Acquisition Manager" : allFU ? "every metric, by Follow-Up Specialist" : "every metric, by rep — Acquisition Managers, then Follow-Up Specialists"}>
          <VpPerVpTable perVp={perRep} groups={AMFU_GROUPS} />
        </VpCard>)}
    </div>);
}

/* ── Marketing cost attribution (r81) ─────────────────────────────────────────────────────────────
 * Spend  = QuickBooks "Vendor Expenses by Month" pivot (PT2 Marketing · "Marketing Spend COGS x …"), mapped to
 *          lead source(s) by the "Vendor x Lead Source - Context" tab:
 *            · Software / Service vendors → charged straight to their listed source (even split if several).
 *            · Data vendors (shared)      → each month's bill split across its listed sources in proportion to each
 *              source's Software+Service spend over the trailing MKT_COST.weightMonths (3) months ending that month.
 *            · Unmapped vendors           → "Unattributed" (never dropped, so totals tie to QuickBooks).
 * Labor  = Rippling contractor ledger ("All Contractor Payments x …") → payee matched to the "Marketing Spend -
 *          Roster" employee by legal first + last name tokens → title keyword → source. Payees not on the roster
 *          are not marketing cost and are excluded. Each payment is spread over its pay period's business days.
 * All facts are kept at month (vendor) / pay-period (labor) grain and prorated by business days into any range,
 * so sub-month ranges are estimates and are labeled as such. */
const MKT_COST = {
  directType: /^(software|service)$/i,
  sharedType: /^data$/i,
  titleRules: [{ test: /call\s*center/i, source: "Call Center" }, { test: /texting|\bsms\b/i, source: "Text Message Campaign" }],
  weightMonths: 3, // trailing window for shared-cost weights: long enough to smooth lumpy dialer billing, short enough that a paused channel (e.g. Direct Mail, dormant Nov-25→Aug-26) stops absorbing shared data cost
  windows: { "Direct Mail Campaign": "lifetime" }, // per-source attribution window; anything else = "period"
  unattributed: "Unattributed",
  // Pre-Rippling (Wise) ledger lines: the memo is the payee's name, or a group label with no name. Group labels
  // are routed here; a named payee still goes through the roster. Anything else unnamed can't be attributed.
  wiseGroupRules: [{ test: /^tier\s*1$/i, source: "Call Center", label: "Tier 1 (unnamed Wise payments)" }],
  missingBillLookback: 3, // a vendor billed in each of the prior N months but not the current month → "no bill posted yet"
};
const MONTHS3 = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"];
const monIdx = (s) => MONTHS3.indexOf(String(s || "").slice(0, 3).toLowerCase());
const srcLabel = (s) => String(s ?? "").trim() || "(No lead source)";
const srcKey = (s) => srcLabel(s).toLowerCase();
const mktVendorKey = (s) => String(s ?? "").toLowerCase().replace(/[^a-z0-9]/g, "").replace(/(llc|inc|corp)$/, "");
const nameTokens = (s) => String(s ?? "").normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z ]/g, " ").split(/\s+/).filter(Boolean);
const ymKey = (d) => d.getFullYear() * 12 + d.getMonth();
const ymDate = (k) => new Date(Math.floor(k / 12), k % 12, 1);
// Pivot month header → first of that month. Handles "Jan 2026" / "January 2026" (Sheets FORMATTED_STRING),
// ISO / M/D/YYYY strings, and serial numbers (as number or numeric string). Anything else (Vendor, Total) → null.
function monthFromHeader(h) {
  if (h == null || h === "") return null;
  const s = String(h).trim();
  let m = s.match(/^([A-Za-z]{3,9})\.?[\s-]+(\d{4})$/);
  if (m && monIdx(m[1]) >= 0) return new Date(+m[2], monIdx(m[1]), 1);
  if (/^\d{5}(\.\d+)?$/.test(s)) { const d = new Date(Date.UTC(1899, 11, 30) + Math.round(+s) * 86400000); return new Date(d.getUTCFullYear(), d.getUTCMonth(), 1); }
  if (/^\d{4}[-/]\d{1,2}([-/]\d{1,2})?$/.test(s) || /^\d{1,2}[/-]\d{1,2}[/-]\d{2,4}$/.test(s)) { const d = parseDate(s.length <= 7 ? s + "-01" : s); return d ? new Date(d.getFullYear(), d.getMonth(), 1) : null; }
  return null;
}
// Share of a [pStart,pEnd] period that falls inside [rStart,rEnd], by business days (calendar days if the period has none).
function periodShare(pStart, pEnd, rStart, rEnd) {
  const s = pStart > rStart ? pStart : rStart, e = pEnd < rEnd ? pEnd : rEnd;
  if (sod(s) > sod(e)) return 0;
  const bd = businessDaysBetween(pStart, pEnd);
  if (bd > 0) return businessDaysBetween(s, e) / bd;
  const cal = Math.round((sod(pEnd) - sod(pStart)) / 86400000) + 1;
  return (Math.round((sod(e) - sod(s)) / 86400000) + 1) / cal;
}
// Parse a Rippling journal memo: "[Rippling] Commission - Amount for {Legal Name}. Payroll run: Jul 01 - Jul 16: …".
// Year comes from the posting date (a December period posted in January lands in December).
function parseRipplingMemo(memo, post) {
  const s = String(memo || "");
  const nm = s.match(/Amount for (.+?)\.\s*Payroll run/i); if (!nm) return null;
  const kind = (s.match(/\]\s*(.+?)\s+-\s+Amount for/i) || [])[1] || "Payment";
  let start = null, end = null;
  const pr = s.match(/Payroll run:?\s*([A-Za-z]{3,9})\.?\s+(\d{1,2})\s*-\s*([A-Za-z]{3,9})\.?\s+(\d{1,2})/i);
  if (pr && post && monIdx(pr[1]) >= 0 && monIdx(pr[3]) >= 0) {
    start = new Date(post.getFullYear(), monIdx(pr[1]), +pr[2]);
    if (start - post > 45 * 86400000) start.setFullYear(start.getFullYear() - 1);
    end = new Date(start.getFullYear(), monIdx(pr[3]), +pr[4]);
    if (end < start) end.setFullYear(end.getFullYear() + 1);
  }
  if (!start && post) { start = sod(post); end = sod(post); }
  return { payee: nm[1].replace(/\s+/g, " ").trim(), kind: kind.trim(), start, end };
}
// Pre-Rippling (Wise / Bill / Expense) lines: memo holds the payee's name, sometimes with " - Role" or "(Role)".
// No pay period is recorded, so the line is dated on its posting date.
function parseWiseLine(memo, post) {
  const raw = String(memo || "").trim(); if (!raw || /rippling/i.test(raw)) return null;
  const group = MKT_COST.wiseGroupRules.find((g) => g.test.test(raw));
  if (group) return { group, payee: raw, kind: "Wise payment", start: sod(post), end: sod(post) };
  const name = raw.replace(/\(.*?\)/g, " ").split(/\s+-\s+/)[0].replace(/\s+/g, " ").trim();
  if (!/^[A-Za-zÀ-ÿ'.\- ]+$/.test(name) || name.split(" ").length < 2 || name.split(" ").length > 6 || /^wise\b/i.test(name)) return { unnamed: true };
  return { payee: name, kind: "Wise payment", start: sod(post), end: sod(post) };
}
function mktEarliest(rows, field) { let min = null; (rows || []).forEach((r) => { const d = parseDate(r[field]); if (d && (!min || d < min)) min = d; }); return min; }

// Range-independent model: every spend dollar as a (source, month|pay-period, amount) fact, plus coverage + flags.
function buildMktCostModel(store) {
  const flags = [];
  const vmap = new Map(), conflicts = [];
  (store.mkt_vendor_map || []).forEach((r) => {
    const name = String(r.vendor ?? "").trim(); if (!name) return;
    const k = mktVendorKey(name);
    const sources = String(r.sources ?? "").split(/[,;]/).map((x) => x.trim()).filter(Boolean);
    const type = String(r.type ?? "").trim().toLowerCase();
    const e = { name, sources, type };
    const prev = vmap.get(k);
    if (prev) { if (prev.type !== type || prev.sources.map(srcKey).sort().join("|") !== sources.map(srcKey).sort().join("|")) conflicts.push(`${prev.name} / ${name}`); return; }
    vmap.set(k, e);
  });
  if (conflicts.length) flags.push({ tone: "warn", text: `Vendor rows disagree after name matching (first row used): ${conflicts.join("; ")}` });
  // ── vendor spend: unpivot, sum within a tab, first tab wins across tabs (so an "All Time" + "YTD" pair never double-counts)
  const cells = new Map(); // vkey|ym → { vendor, vkey, ym, amount, tab }
  (store.mkt_spend || []).forEach((row) => {
    const vendor = String(row.Vendor ?? "").trim();
    if (!vendor || /^total\b/i.test(vendor)) return;
    const vkey = mktVendorKey(vendor), tab = row.__tab || "";
    for (const h of Object.keys(row)) {
      const m = monthFromHeader(h); if (!m) continue;
      const amt = Number(row[h]); if (!amt) continue;
      const ck = `${vkey}|${ymKey(m)}`, cur = cells.get(ck);
      if (!cur) cells.set(ck, { vendor, vkey, ym: ymKey(m), amount: amt, tab });
      else if (cur.tab === tab) cur.amount += amt;
    }
  });
  const spend = [...cells.values()];
  // pass 1 — direct (software/service) spend per source per month, used as the weight basis for shared costs
  const directBy = new Map(); // srcKey → Map(ym → amt)
  const addDirect = (s, ym, a) => { const k = srcKey(s); if (!directBy.has(k)) directBy.set(k, new Map()); const m = directBy.get(k); m.set(ym, (m.get(ym) || 0) + a); };
  const unmapped = new Set(), untyped = new Set();
  spend.forEach((c) => {
    const e = vmap.get(c.vkey);
    if (!e || !e.sources.length) return;
    const isShared = MKT_COST.sharedType.test(e.type);
    if (!isShared) { if (!MKT_COST.directType.test(e.type)) untyped.add(e.name); e.sources.forEach((s) => addDirect(s, c.ym, c.amount / e.sources.length)); }
  });
  const trailingDirect = (s, ym) => { const m = directBy.get(srcKey(s)); if (!m) return 0; let t = 0; for (let k = ym - MKT_COST.weightMonths + 1; k <= ym; k++) t += m.get(k) || 0; return t; };
  const facts = [], evenSplitShared = new Set();
  spend.forEach((c) => {
    const e = vmap.get(c.vkey);
    const start = ymDate(c.ym), end = eod(new Date(start.getFullYear(), start.getMonth() + 1, 0));
    const base = { kind: "vendor", vendor: e ? e.name : c.vendor, vkey: c.vkey, ym: c.ym, start, end };
    if (!e || !e.sources.length) { unmapped.add(c.vendor); facts.push({ ...base, source: MKT_COST.unattributed, amount: c.amount, category: "unmapped", weight: 1 }); return; }
    if (MKT_COST.sharedType.test(e.type)) {
      const w = e.sources.map((s) => trailingDirect(s, c.ym)), W = w.reduce((a, b) => a + b, 0);
      if (!W && e.sources.length > 1) evenSplitShared.add(e.name);
      e.sources.forEach((s, i) => { const wt = W ? w[i] / W : 1 / e.sources.length; if (wt) facts.push({ ...base, source: s, amount: c.amount * wt, category: "shared", weight: wt }); });
    } else e.sources.forEach((s) => facts.push({ ...base, source: s, amount: c.amount / e.sources.length, category: "direct", weight: 1 / e.sources.length }));
  });
  if (unmapped.size) flags.push({ tone: "warn", text: `Vendors not in the Vendor x Lead Source tab (shown as Unattributed): ${[...unmapped].join(", ")}` });
  if (untyped.size) flags.push({ tone: "warn", text: `Vendor Type blank or unrecognized (treated as direct): ${[...untyped].join(", ")}` });
  if (evenSplitShared.size) flags.push({ tone: "info", text: `Shared vendors split evenly in months where no listed source had direct spend: ${[...evenSplitShared].join(", ")}` });
  // ── labor: ledger → roster employee → title keyword → source
  const roster = []; const seenEmp = new Set();
  [...(store.mkt_roster || []), ...(store.mkt_roster_add || [])].forEach((r) => { const id = String(r.empId ?? "").trim() || (r.first && r.last ? `manual:${String(r.first).trim()} ${String(r.last).trim()}`.toLowerCase() : ""); if (!id || seenEmp.has(id)) return; seenEmp.add(id);
    const title = String(r.title ?? "").trim(); const rule = MKT_COST.titleRules.find((x) => x.test.test(title));
    roster.push({ id, name: String(r.name ?? "").trim() || `${r.first} ${r.last}`, first: nameTokens(r.first), last: nameTokens(r.last), title, source: rule ? rule.source : MKT_COST.unattributed }); });
  const ambiguous = new Set(), badTitles = new Set(); let nonRoster = 0, unnamed = 0, laborStart = null, laborEnd = null, ripplingStart = null;
  (store.mkt_labor || []).forEach((r) => {
    const amt = Number(r.amount); if (!amt) return;
    const post = parseDate(r.postDate); if (!post) return;
    let p = parseRipplingMemo(r.memo, post);
    if (p) { if (!ripplingStart || p.start < ripplingStart) ripplingStart = p.start; }
    else {
      p = parseWiseLine(r.memo, post); if (!p) return;
      if (p.unnamed) { if (/^wise\b/i.test(String(r.payee ?? "").trim())) unnamed += amt; else nonRoster += amt; return; } // non-Wise bills (e.g. EOR vendors) aren't roster pay
      if (p.group) { if (!laborStart || p.start < laborStart) laborStart = p.start;
        facts.push({ kind: "labor", source: p.group.source, amount: amt, category: "labor", empId: `group:${p.group.label}`, employee: p.group.label, title: "Wise era", payKind: p.kind, start: p.start, end: eod(p.end) }); return; }
    }
    const tk = new Set(nameTokens(p.payee));
    const hits = roster.filter((e) => e.first.length && e.last.length && e.first.every((t) => tk.has(t)) && e.last.every((t) => tk.has(t)));
    if (!hits.length) { nonRoster += amt; return; }
    if (hits.length > 1 && new Set(hits.map((h) => h.title)).size > 1) { ambiguous.add(p.payee); return; }
    const e = hits[0];
    if (e.source === MKT_COST.unattributed) badTitles.add(e.title || "(blank)");
    if (!laborStart || p.start < laborStart) laborStart = p.start;
    if (!laborEnd || p.end > laborEnd) laborEnd = p.end;
    facts.push({ kind: "labor", source: e.source, amount: amt, category: "labor", empId: e.id, employee: e.name, title: e.title, payKind: p.kind, start: sod(p.start), end: eod(p.end) });
  });
  if (unnamed) flags.push({ tone: "info", text: `${mkt$0(unnamed)} of pre-Rippling contractor transfers carry no payee name (e.g. "WISE INC") and can't be attributed — excluded.` });
  if (ambiguous.size) flags.push({ tone: "warn", text: `Payees matching more than one roster employee with different titles (excluded): ${[...ambiguous].join(", ")}` });
  if (badTitles.size) flags.push({ tone: "warn", text: `Roster titles with no lead-source keyword (labor shown as Unattributed): ${[...badTitles].join(", ")}` });
  // ── outcome datasets (closed = YTD tab ∪ any historical tab, one row per Opportunity ID)
  const closedSeen = new Set(), closed = [];
  [...(store.closed_opps || []), ...(store.closed_opps_hist || [])].forEach((r) => { const id = String(r.id ?? "").trim(); const k = id || `${r.name}|${r.closeDate}`; if (closedSeen.has(k)) return; closedSeen.add(k); closed.push(r); });
  const coverage = {
    closed: mktEarliest(closed, "closeDate"), opps: mktEarliest(store.mkt_opps, "date"), arip: mktEarliest(store.arip_entered, "date"), revives: mktEarliest(store.mkt_revives, "date"),
    leadsBySource: (() => { const m = new Map(); (store.leads || []).forEach((r) => { const d = parseDate(r.date); const k = srcKey(r.source); if (d && (!m.has(k) || d < m.get(k))) m.set(k, d); }); return m; })(),
    labor: laborStart, laborEnd, rippling: ripplingStart,
  };
  // Labor run-rate per source over the last 30 days the ledger covers — used to estimate pay not yet posted.
  const laborRate = new Map();
  if (laborEnd) {
    const ws = sod(addDays(laborEnd, -29)), we = eod(laborEnd), bd = businessDaysBetween(ws, we) || 1;
    facts.forEach((f) => { if (f.kind !== "labor") return; const a = f.amount * periodShare(f.start, f.end, ws, we); if (!a) return;
      const k = srcKey(f.source); const x = laborRate.get(k) || { source: f.source, rate: 0 }; x.rate += a / bd; laborRate.set(k, x); });
  }
  const firstSpend = new Map(); facts.forEach((f) => { if (f.kind !== "vendor") return; const k = srcKey(f.source); if (!firstSpend.has(k) || f.start < firstSpend.get(k)) firstSpend.set(k, f.start); });
  const displayName = new Map(); const nameOf = (s) => { const k = srcKey(s); if (!displayName.has(k)) displayName.set(k, srcLabel(s)); return displayName.get(k); };
  facts.forEach((f) => nameOf(f.source));
  return { facts, flags, coverage, closed, firstSpend, nameOf, nonRoster, laborRate, rosterCount: roster.length, vendorsMapped: vmap.size, hasSpend: spend.length > 0 };
}

// Customer grouping: a back-end joins its front-end by Opportunity Name (after stripping "Back-End ~").
const custKey = (n) => String(n ?? "").replace(/^\s*back[\s-]*end\s*~\s*/i, "").toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 25);
const isListingRT = (r) => /listing/i.test(String(r.recordType ?? ""));
// Group closed deals into customers. Returns { core: [{src, rev}], listing: [{src, rev}], orphanBackRev }.
// core customer = a group with a non-back-end deal. Back-end-only groups add revenue but no new customer.
function mktCustomers(rows) {
  const groups = new Map(), listing = [];
  rows.forEach((r) => {
    const rev = num(r.revenue);
    if (isListingRT(r)) { listing.push({ src: r.source, rev }); return; }
    const k = custKey(r.name) || String(r.id); if (!groups.has(k)) groups.set(k, { src: null, rev: 0, front: false, back: false, other: false });
    const g = groups.get(k); g.rev += rev;
    if (isBackEndRT(r)) g.back = true;
    else { g.front = true; if (!isFrontEndRT(r)) g.other = true; if (g.src == null || !isFrontEndRT(r)) g.src = r.source; }
  });
  const core = [], backOnly = [];
  groups.forEach((g) => { if (g.front) core.push(g); else backOnly.push(g); });
  // A Purchase front-end with no back-end yet and $0 fee is unrealized (the fee lands at back-end sale):
  // it counts as an acquired customer for CAC but stays out of the LTV average until the back-end closes.
  core.forEach((g) => { g.unrealized = !g.back && !g.other && g.rev === 0; });
  return { core, listing, backOnly };
}

// Everything the efficiency panel shows for one range. Range-dependent; model is range-independent.
function mktEfficiency(model, store, range, rangeFwd, opts, now = new Date()) {
  const withLabor = !!opts.withLabor;
  const today = eod(now), rEnd = range.end < today ? range.end : today;
  const flags = [];
  const monthAligned = range.start.getDate() === 1 && eod(range.end).getTime() === eom(range.end).getTime();
  if (!monthAligned) flags.push({ tone: "info", text: "Estimated — spend is monthly (vendors) / per pay period (labor), prorated by business days into this range." });
  // per-source window
  const windowFor = (s) => {
    const k = srcKey(s);
    const mode = Object.entries(MKT_COST.windows).find(([n]) => srcKey(n) === k)?.[1];
    // The main row always follows the selected period; a lifetime source also gets a "since first spend" row,
    // where each metric turns on only once its own dataset reaches back to that first spend (see lifeFor).
    return { start: range.start, mode: "period", lifetime: mode === "lifetime" ? model.firstSpend.get(k) || null : null };
  };
  const inWin = (d, w) => { const t = parseDate(d); return !!t && t >= w.start && t <= rEnd; };
  // estimates for costs not posted yet (current month only; opts.estimate !== false)
  const curYm = ymKey(now), est = [], estNotes = [];
  const billed = new Map(); model.facts.forEach((f) => { if (f.kind !== "vendor") return; if (!billed.has(f.vendor)) billed.set(f.vendor, new Set()); billed.get(f.vendor).add(f.ym); });
  const missingBills = rEnd >= som(now) ? [...billed.entries()].filter(([, s]) => !s.has(curYm) && Array.from({ length: MKT_COST.missingBillLookback }, (_, i) => curYm - 1 - i).every((k) => s.has(k))).map(([v]) => v) : [];
  if (opts.estimate !== false) {
    const mS = som(now), mE = eom(now), L = MKT_COST.missingBillLookback;
    missingBills.forEach((v) => {
      const agg = new Map(); let tot = 0;
      model.facts.forEach((f) => { if (f.kind !== "vendor" || f.vendor !== v || f.ym < curYm - L || f.ym > curYm - 1) return;
        const k = `${srcKey(f.source)}|${f.category}`; const x = agg.get(k) || { ...f, amount: 0 }; x.amount += f.amount / L; agg.set(k, x); tot += f.amount / L; });
      agg.forEach((x) => est.push({ ...x, ym: curYm, start: mS, end: mE, estimated: true }));
      estNotes.push(`${v} ${mkt$0(tot)} (avg of prior ${L} months)`);
    });
    const le = model.coverage.laborEnd;
    if (withLabor && le && rEnd > eod(le)) {
      const s0 = sod(addDays(le, 1)), bd = businessDaysBetween(s0, today);
      model.laborRate.forEach((x) => { if (x.rate > 0 && bd > 0) est.push({ kind: "labor", source: x.source, amount: x.rate * bd, category: "labor", empId: "__est", employee: "Unposted pay (estimate)", title: `run-rate ${mkt$0(x.rate)}/business day`, start: s0, end: today, estimated: true }); });
    }
  }
  const allFacts = est.length ? model.facts.concat(est) : model.facts;
  // cost
  const srcs = new Map(); // srcKey → bucket
  const vendorTot = new Map(); // shared vendor key → its total in range (all sources), for the drill-down share %
  const bucket = (s) => { const k = srcKey(s); if (!srcs.has(k)) srcs.set(k, { key: k, name: model.nameOf(s), win: windowFor(s), cost: 0, vendor: 0, labor: 0, est: 0, parts: new Map() }); return srcs.get(k); };
  allFacts.forEach((f) => {
    if (f.kind === "labor" && !withLabor) return;
    const b = bucket(f.source); const sh = periodShare(f.start, f.end, b.win.start, rEnd); if (!sh) return;
    const a = f.amount * sh; b.cost += a; if (f.kind === "labor") b.labor += a; else b.vendor += a; if (f.estimated) b.est += a;
    const pk = (f.kind === "labor" ? `L|${f.empId}` : `${f.category}|${f.vkey}`) + (f.estimated ? "|est" : "");
    const p = b.parts.get(pk) || { label: f.kind === "labor" ? f.employee : f.vendor, sub: (f.kind === "labor" ? f.title : f.category === "shared" ? "shared data" : f.category === "unmapped" ? "unmapped vendor" : "direct") + (f.estimated && f.kind !== "labor" ? ` · estimated, ${MKT_COST.missingBillLookback}-mo avg` : ""), kind: f.kind, estimated: !!f.estimated, amount: 0 };
    p.amount += a; if (f.category === "shared") { p.vkey = f.vkey; vendorTot.set(f.vkey, (vendorTot.get(f.vkey) || 0) + a); } b.parts.set(pk, p);
  });
  srcs.forEach((b) => b.parts.forEach((p) => { if (p.vkey && vendorTot.get(p.vkey)) p.share = p.amount / vendorTot.get(p.vkey); }));
  // outcomes
  const leadsBy = new Map(), blendBy = new Map(), oppsBy = new Map(), aripBy = new Map(), closedBy = new Map(), openBy = new Map();
  const addSet = (m, s, id) => { const k = srcKey(s); if (!m.has(k)) m.set(k, new Set()); m.get(k).add(id); };
  (store.leads || []).forEach((r) => { if (!r.leadId) return; const b = bucket(r.source); if (inWin(r.date, b.win)) { addSet(leadsBy, r.source, r.leadId); addSet(blendBy, r.source, r.leadId); } });
  (store.mkt_revives || []).forEach((r) => { if (!r.leadId || !/^dead$/i.test(String(r.oldValue ?? "").trim())) return; const b = bucket(r.source); if (inWin(r.date, b.win)) addSet(blendBy, r.source, r.leadId); });
  (store.mkt_opps || []).forEach((r) => { if (!r.id) return; const b = bucket(r.source); if (inWin(r.date, b.win)) addSet(oppsBy, r.source, r.id); });
  (store.arip_entered || []).forEach((r) => { if (!r.id) return; const b = bucket(r.source); if (inWin(r.date, b.win)) addSet(aripBy, r.source, r.id); });
  model.closed.forEach((r) => { const b = bucket(r.source); if (inWin(r.closeDate, b.win)) { const k = srcKey(r.source); if (!closedBy.has(k)) closedBy.set(k, []); closedBy.get(k).push(r); } });
  const fwdEnd = rangeFwd && rangeFwd.end > rEnd ? rangeFwd.end : rEnd; let pastDue = 0;
  (store.pipeline || []).forEach((r) => { if (!isOpen(r.stage)) return; const b = bucket(r.source); const d = parseDate(r.closeDate); if (!d || d < b.win.start || d > fwdEnd) return;
    const k = srcKey(r.source); const o = openBy.get(k) || { rev: 0, n: 0, pastDue: 0 }; o.rev += num(r.forecast); o.n++; if (d < sod(now)) { o.pastDue++; pastDue++; } openBy.set(k, o); });
  // LTV constant: trailing 12 months when closed history reaches back that far, else YTD (as of today)
  const t12Start = sod(addDays(now, -364)), ytdStart = new Date(now.getFullYear(), 0, 1);
  const ltvMode = model.coverage.closed && model.coverage.closed <= addDays(t12Start, 31) ? "T12" : "YTD";
  const ltvStart = ltvMode === "T12" ? t12Start : ytdStart;
  const ltvRows = model.closed.filter((r) => { const d = parseDate(r.closeDate); return d && d >= ltvStart && d <= today; });
  const ltvCust = mktCustomers(ltvRows).core.filter((g) => !g.unrealized);
  const ltvCompany = ltvCust.length ? ltvCust.reduce((s, g) => s + g.rev, 0) / ltvCust.length : null;
  const ltvBySrc = new Map(); ltvCust.forEach((g) => { const k = srcKey(g.src); const x = ltvBySrc.get(k) || { rev: 0, n: 0 }; x.rev += g.rev; x.n++; ltvBySrc.set(k, x); });
  const ltvUnrealized = mktCustomers(ltvRows).core.length - ltvCust.length;
  const div = (a, b) => (b ? a / b : null);
  const cdiv = (c, n) => (c > 0.005 && n ? c / n : null); // cost-per metrics are blank (not $0) when there's no spend
  const covOk = (d, fs) => !!d && d - fs <= 31 * 86400000;
  const cohortMode = opts.attribution === "cohort";
  const cohortFor = (k, start) => {
    const ids = new Set(); let converted = 0, linked = 0, convertedLinked = 0;
    (store.leads || []).forEach((r) => { if (!r.leadId || srcKey(r.source) !== k) return; const t = parseDate(r.date); if (!t || t < start || t > rEnd) return;
      const conv = /convert/i.test(String(r.status ?? "")), id = id15(r.oppId);
      if (conv) converted++; if (id) { linked++; ids.add(id); if (conv) convertedLinked++; } });
    const closed = [], seenBack = new Set();
    ids.forEach((id) => { const c = cohortIdx.closedById.get(id); if (c) closed.push(c);
      const nk = cohortIdx.oppName.get(id) || (c ? custKey(c.name) : null);
      if (nk && !seenBack.has(nk)) { seenBack.add(nk); (cohortIdx.backByKey.get(nk) || []).forEach((b2) => closed.push(b2)); } });
    return { opps: ids.size, arips: [...ids].filter((id) => cohortIdx.aripIds.has(id)).length, closed, converted, linked, convertedLinked };
  };
  const lifeFor = (b) => {
    const fs = b.win.lifetime; if (!fs) return null;
    const k = b.key, cv = model.coverage, inL = (d) => { const t = parseDate(d); return !!t && t >= fs && t <= rEnd; };
    let cost = 0; allFacts.forEach((f) => { if ((f.kind === "labor" && !withLabor) || srcKey(f.source) !== k) return; cost += f.amount * periodShare(f.start, f.end, fs, rEnd); });
    const L = { key: `${k}|life`, isLife: true, name: `↳ Lifetime since ${fs.toLocaleDateString(undefined, { month: "short", year: "numeric" })}`, cost, pending: new Set() };
    const uniq = (rows, idF, srcF, dateF) => { const x = new Set(); (rows || []).forEach((r) => { if (r[idF] && srcKey(r[srcF]) === k && inL(r[dateF])) x.add(r[idF]); }); return x; };
    if (covOk(cv.leadsBySource.get(k), fs)) { const ld = uniq(store.leads, "leadId", "source", "date"); L.leads = ld.size; L.cpl = cdiv(cost, ld.size);
      if (covOk(cv.revives, fs)) { const bl = new Set(ld); (store.mkt_revives || []).forEach((r) => { if (r.leadId && /^dead$/i.test(String(r.oldValue ?? "").trim()) && srcKey(r.source) === k && inL(r.date)) bl.add(r.leadId); }); L.blended = bl.size; L.cplBlended = cdiv(cost, bl.size); } else L.pending.add("revives");
    } else L.pending.add("leads");
    const lc = cohortMode && !L.pending.has("leads") ? cohortFor(k, fs) : null;
    if (lc) { L.opps = lc.opps; L.cpo = cdiv(cost, L.opps); L.arips = lc.arips; L.cpa = cdiv(cost, L.arips); }
    else {
      if (covOk(cv.opps, fs)) { L.opps = uniq(store.mkt_opps, "id", "source", "date").size; L.cpo = cdiv(cost, L.opps); } else L.pending.add("opps");
      if (covOk(cv.arip, fs)) { L.arips = uniq(store.arip_entered, "id", "source", "date").size; L.cpa = cdiv(cost, L.arips); } else L.pending.add("arip");
    }
    if (lc || covOk(cv.closed, fs)) {
      const cl = lc ? lc.closed : model.closed.filter((r) => srcKey(r.source) === k && inL(r.closeDate)); const cu = mktCustomers(cl);
      L.closedRev = cl.reduce((a, r) => a + num(r.revenue), 0); L.core = cu.core.length; L.listing = cu.listing.length;
      let open = 0; (store.pipeline || []).forEach((r) => { if (!isOpen(r.stage) || srcKey(r.source) !== k) return; const d = parseDate(r.closeDate); if (d && d >= fs && d <= fwdEnd) open += num(r.forecast); });
      if (lc) open = 0; L.openRev = open; L.cac = cdiv(cost, L.core); L.cacBlended = cdiv(cost, L.core + L.listing); L.roas = div(L.closedRev, cost); L.roasFc = lc ? null : div(L.closedRev + open, cost);
    } else L.pending.add("closed");
    return L;
  };
  // Lead-cohort attribution: follow the leads created in the window (by Lead Create Date) through the Opportunity ID
  // on the lead row to their opps, ARIPs and closed deals — whenever those happened (through today). A back-end joins
  // its front-end by name (same rule as customers). Coverage = converted leads that carry an Opp ID.
  const id15 = (v) => String(v ?? "").trim().slice(0, 15);
  const cohortIdx = cohortMode ? (() => {
    const oppName = new Map(); (store.mkt_opps || []).forEach((r) => { if (r.id) oppName.set(id15(r.id), custKey(r.name)); });
    const aripIds = new Set(); (store.arip_entered || []).forEach((r) => { const d = parseDate(r.date); if (r.id && d && d <= today) aripIds.add(id15(r.id)); });
    const closedById = new Map(), backByKey = new Map();
    model.closed.forEach((r) => { const d = parseDate(r.closeDate); if (!d || d > today) return;
      if (isBackEndRT(r)) { const k2 = custKey(r.name); if (!backByKey.has(k2)) backByKey.set(k2, []); backByKey.get(k2).push(r); }
      else if (r.id) closedById.set(id15(r.id), r); });
    return { oppName, aripIds, closedById, backByKey };
  })() : null;
  // rows
  const rows = [...srcs.values()].map((b) => {
    const k = b.key, co = cohortMode ? cohortFor(k, b.win.start) : null, cl = co ? co.closed : closedBy.get(k) || [];
    const cu = mktCustomers(cl);
    const closedRev = cl.reduce((s, r) => s + num(r.revenue), 0), open = co ? { rev: 0, n: 0, pastDue: 0 } : openBy.get(k) || { rev: 0, n: 0, pastDue: 0 };
    const hasLeadTab = model.coverage.leadsBySource.has(k); // no new-lead report for this source → no CPL / blended CPL
    const leads = (leadsBy.get(k) || new Set()).size, blended = hasLeadTab ? (blendBy.get(k) || new Set()).size : 0;
    const opps = co ? co.opps : (oppsBy.get(k) || new Set()).size, arips = co ? co.arips : (aripBy.get(k) || new Set()).size;
    const core = cu.core.length, listing = cu.listing.length;
    const sl = ltvBySrc.get(k); const ltvSrc = sl && sl.n ? sl.rev / sl.n : null;
    const ltv = opts.ltvBasis === "source" ? ltvSrc : ltvCompany;
    const cac = cdiv(b.cost, core);
    return { ...b, parts: [...b.parts.values()].sort((x, y) => y.amount - x.amount), leads, blended, opps, arips, core, listing, closedRev, openRev: open.rev, openN: open.n, openPastDue: open.pastDue,
      cpl: cdiv(b.cost, leads), cplBlended: cdiv(b.cost, blended), cpo: cdiv(b.cost, opps), cpa: cdiv(b.cost, arips), cac, cacBlended: cdiv(b.cost, core + listing),
      ltv, ltvN: opts.ltvBasis === "source" ? (sl ? sl.n : 0) : ltvCust.length, ltvCac: ltv != null && cac ? ltv / cac : null,
      roas: div(closedRev, b.cost), roasFc: co ? null : div(closedRev + open.rev, b.cost), life: lifeFor(b), cohort: co };
  });
  rows.forEach((r) => { if (r.life && r.life.core != null) { r.life.ltv = r.ltv; r.life.ltvN = r.ltvN; r.life.ltvCac = r.ltv != null && r.life.cac ? r.ltv / r.life.cac : null; } });
  // a lifetime source stays listed even in a period with no spend of its own, so its lifetime row is always visible
  const paid = rows.filter((r) => (r.cost > 0.005 || (r.life && r.life.cost > 0.005)) && r.key !== srcKey(MKT_COST.unattributed)).sort((a, b) => b.cost - a.cost);
  const unattr = rows.find((r) => r.key === srcKey(MKT_COST.unattributed) && r.cost > 0.005) || null;
  const unpaid = rows.filter((r) => !(r.cost > 0.005) && !(r.life && r.life.cost > 0.005) && r.key !== srcKey(MKT_COST.unattributed) && (r.closedRev || r.openRev)).sort((a, b) => (b.closedRev + b.openRev) - (a.closedRev + a.openRev));
  const sum = (f) => paid.reduce((s, r) => s + (r[f] || 0), 0);
  const tCost = sum("cost") + (unattr ? unattr.cost : 0), tCore = sum("core"), tRev = sum("closedRev"), tOpen = sum("openRev");
  const total = { cost: tCost, est: sum("est") + (unattr ? unattr.est : 0), vendor: sum("vendor") + (unattr ? unattr.vendor : 0), labor: sum("labor") + (unattr ? unattr.labor : 0), leads: sum("leads"), blended: sum("blended"), opps: sum("opps"), arips: sum("arips"), core: tCore, listing: sum("listing"), closedRev: tRev, openRev: tOpen, openN: sum("openN"),
    cpl: div(tCost, sum("leads")), cplBlended: div(tCost, sum("blended")), cpo: div(tCost, sum("opps")), cpa: div(tCost, sum("arips")), cac: div(tCost, tCore), cacBlended: div(tCost, tCore + sum("listing")),
    ltv: ltvCompany, ltvN: ltvCust.length, ltvCac: ltvCompany != null && tCore && tCost ? ltvCompany / (tCost / tCore) : null, roas: div(tRev, tCost), roasFc: cohortMode ? null : div(tRev + tOpen, tCost) };
  // flags: costs not posted yet (estimated, or understated when estimates are off)
  const mon = now.toLocaleDateString(undefined, { month: "short" });
  const le = model.coverage.laborEnd, leTxt = le ? le.toLocaleDateString(undefined, { month: "short", day: "numeric" }) : "";
  if (missingBills.length) flags.push(opts.estimate !== false
    ? { tone: "info", text: `Estimated — no ${mon} bill posted yet, filled with the vendor's average: ${estNotes.join("; ")}. Replaced automatically once the bill posts.` }
    : { tone: "warn", text: `No ${mon} bill posted yet for ${missingBills.join(", ")} — billed each of the prior ${MKT_COST.missingBillLookback} months, so this month's cost for their source is understated.` });
  const laborGap = withLabor && le && rEnd > eod(le);
  if (laborGap && opts.estimate !== false) flags.push({ tone: "info", text: `Estimated — contractor pay is recorded through ${leTxt}; later days use each source's run-rate from the last 30 recorded days (${[...model.laborRate.values()].filter((x) => x.rate > 0).map((x) => `${model.nameOf(x.source)} ${mkt$0(x.rate)}/business day`).join(", ")}). Replaced as payroll posts.` });
  else if (laborGap) flags.push({ tone: "warn", text: `Labor recorded through ${leTxt} — pay for later days in this range isn't in the ledger yet, so labor is understated.` });
  if (withLabor && model.coverage.rippling && range.start < model.coverage.rippling && rEnd > (model.coverage.labor || model.coverage.rippling)) flags.push({ tone: "warn", text: `Labor before ${model.coverage.rippling.toLocaleDateString(undefined, { month: "short", year: "numeric" })} (Wise era) is partial — it covers only people on the roster today plus "Tier 1" payments (→ Call Center); former staff aren't on the Rippling roster, and Wise lines are dated on payment date, not pay period.` });
  if (withLabor && model.coverage.labor && range.start < model.coverage.labor) flags.push({ tone: "info", text: `Labor unavailable before ${model.coverage.labor.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })} — earlier contractor pay (Wise era) carries no pay period and mostly goes to people not on the roster, so earlier dates are vendor spend only.` });
  const PEND = { leads: "leads", revives: "revives (blended CPL)", opps: "opportunities (cost / opp)", arip: "ARIPs (cost / ARIP)", closed: "closed deals (CAC, LTV:CAC, ROAS)" };
  const covTxt = { leads: null, revives: model.coverage.revives, opps: model.coverage.opps, arip: model.coverage.arip, closed: model.coverage.closed };
  paid.forEach((r) => { if (r.life && r.life.pending.size) flags.push({ tone: "info", text: `${r.name} lifetime row: ${["leads", "revives", "opps", "arip", "closed"].filter((x) => !r.life.pending.has(x) && x !== "revives").map((x) => PEND[x].split(" (")[0]).join(", ") || "nothing"} live since first spend; pending history for ${[...r.life.pending].map((x) => `${PEND[x]}${covTxt[x] ? ` — data starts ${covTxt[x].toLocaleDateString(undefined, { month: "short", year: "numeric" })}` : ""}`).join("; ")}.` }); });
  if (cohortMode) {
    const cov = paid.filter((r) => r.cohort && r.cohort.converted).map((r) => `${r.name} ${r.cohort.convertedLinked}/${r.cohort.converted} (${Math.round((r.cohort.convertedLinked / r.cohort.converted) * 100)}%)`);
    const tc = paid.reduce((a, r) => a + (r.cohort ? r.cohort.converted : 0), 0), tl = paid.reduce((a, r) => a + (r.cohort ? r.cohort.convertedLinked : 0), 0);
    flags.unshift({ tone: tc && tl / tc >= 0.9 ? "info" : "warn", text: `Lead cohort view — outcomes follow the leads created in this range via the Opportunity ID on each lead. Converted leads carrying an Opp ID: ${cov.join(" · ") || "none"}.${tc && tl / tc < 0.9 ? " Opps, ARIPs, customers and ROAS are understated until the lead reports are refreshed with Opp IDs." : ""} Forecast ROAS isn't available in this view (the pipeline report has no Opportunity ID).` });
  }
  if (pastDue && !cohortMode) flags.push({ tone: "info", text: `Forecast ROAS includes ${pastDue} open deal${pastDue === 1 ? "" : "s"} whose Close Date has passed but hasn't closed yet.` });
  const lead = cohortMode ? flags.splice(0, 1) : []; // the cohort coverage banner leads the list
  return { paid, unattr, unpaid, total, flags: [...lead, ...model.flags, ...flags], ltvMode, ltvStart, ltvCompany, ltvN: ltvCust.length, ltvUnrealized, monthAligned };
}

const mkt$ = (v) => (v == null || !isFinite(v) ? "—" : (v < 0 ? "-$" : "$") + Math.abs(v).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
const mkt$0 = (v) => (v == null || !isFinite(v) ? "—" : fmt(v, "currency"));
const mktX = (v) => (v == null || !isFinite(v) ? "—" : (v >= 100 ? Math.round(v) : v.toFixed(1)) + "x");
const mktN = (v) => (v ? v.toLocaleString() : "—");
function MktSeg({ value, onChange, options, label }) {
  return (<div className="flex items-center gap-2">
    <span className="text-[11px] uppercase tracking-wide" style={{ color: T.faint }}>{label}</span>
    <div className="inline-flex rounded-xl p-0.5" style={{ background: T.track }}>
      {options.map(([v, l]) => (
        <button key={String(v)} onClick={() => onChange(v)} className="text-[12px] font-medium px-3 py-1 rounded-md transition-colors whitespace-nowrap"
          style={{ background: value === v ? T.card : "transparent", color: value === v ? T.ink : T.sub, boxShadow: value === v ? "0 1px 2px rgba(0,0,0,0.06)" : "none" }}>{l}</button>))}
    </div></div>);
}
function MktTable({ cols, rows, total, openKey, setOpenKey, withLabor }) {
  const th = { borderBottom: `1px solid ${T.border}` }, td = { borderBottom: `1px solid ${T.border}`, fontVariantNumeric: "tabular-nums" };
  const line = (r, isTotal) => (
    <tr key={r.key || "total"} className={isTotal ? "" : "lh-row"} onClick={isTotal ? undefined : () => setOpenKey(openKey === r.key ? null : r.key)}
      style={{ color: T.ink, cursor: isTotal ? "default" : "pointer", fontWeight: isTotal ? 700 : 400 }}>
      <td className="py-2 px-2 whitespace-nowrap" style={{ ...td, fontWeight: 600 }}>
        {!isTotal && <span style={{ color: T.faint, display: "inline-block", width: 12 }}>{openKey === r.key ? "▾" : "▸"}</span>}{r.name}
        {r.life && r.life.pending.has("closed") && <span className="ml-1.5 text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: T.warnSoft, color: T.warn }} title="Long response lag — the period's revenue comes largely from earlier spend; see the lifetime row">lag ⚠</span>}
      </td>
      {cols.map((c) => <td key={c.key} className="py-2 px-2 text-right whitespace-nowrap" style={{ ...td, color: c.muted ? T.sub : T.ink }}>{c.f(r)}</td>)}
    </tr>);
  return (<div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
    <table className="w-full text-[13px]" style={{ borderCollapse: "collapse", minWidth: 760 }}>
      <thead><tr className="text-[11px] uppercase tracking-wide" style={{ color: T.faint }}>
        <th className="py-2 px-2 text-left" style={th}>Lead source</th>
        {cols.map((c) => <th key={c.key} className="py-2 px-2 text-right whitespace-nowrap" style={th} title={c.tip}>{c.label}</th>)}
      </tr></thead>
      <tbody>
        {rows.map((r) => (<React.Fragment key={r.key}>
          {line(r)}
          {r.life && (<tr style={{ color: T.sub }}>
            <td className="py-1.5 px-2 whitespace-nowrap text-[12px]" style={{ ...td, paddingLeft: 22, fontStyle: "italic" }}>{r.life.name}</td>
            {cols.map((c) => <td key={c.key} className="py-1.5 px-2 text-right whitespace-nowrap text-[12px]" style={{ ...td, color: T.sub }}>
              {c.need && c.need.some((n) => r.life.pending.has(n)) ? <span style={{ color: T.faint }}>pending</span> : c.f(r.life)}</td>)}
          </tr>)}
          {openKey === r.key && (<tr><td colSpan={cols.length + 1} className="px-3 py-2" style={{ background: T.zebra, borderBottom: `1px solid ${T.border}` }}>
            <div className="text-[11px] mb-1" style={{ color: T.faint }}>Cost components in this range{withLabor ? "" : " (labor off)"} — {mkt$0(r.cost)} total{r.labor ? ` · vendor ${mkt$0(r.vendor)} + labor ${mkt$0(r.labor)}` : ""}</div>
            <div className="grid gap-x-6 gap-y-0.5" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))" }}>
              {r.parts.map((p) => (<div key={p.label + p.sub} className="flex justify-between text-[12px]" style={{ color: T.ink }}>
                <span style={{ fontStyle: p.estimated ? "italic" : "normal" }}>{p.label} <span style={{ color: T.faint }}>· {p.sub}{p.share != null ? ` · ${Math.round(p.share * 1000) / 10}% of its bill` : ""}</span></span>
                <span style={{ fontVariantNumeric: "tabular-nums" }}>{mkt$0(p.amount)}</span></div>))}
            </div></td></tr>)}
        </React.Fragment>))}
        {total && line({ ...total, name: "All paid sources" }, true)}
      </tbody>
    </table></div>);
}
function MarketingEfficiency({ store, range, rangeFwd }) {
  const [withLabor, setWithLabor] = useState(true);
  const [ltvBasis, setLtvBasis] = useState("company");
  const [estimate, setEstimate] = useState(true); // fill not-yet-posted bills / payroll with averages
  const [attribution, setAttribution] = useState("event"); // "event" = each outcome on its own date · "cohort" = follow the period's leads
  const [openKey, setOpenKey] = useState(null);
  const model = useMemo(() => buildMktCostModel(store), [store]);
  const res = useMemo(() => (model.hasSpend ? mktEfficiency(model, store, range, rangeFwd, { withLabor, ltvBasis, estimate, attribution }) : null), [model, store, range, rangeFwd, withLabor, ltvBasis, estimate, attribution]);
  if (!res) return (<Panel title="Spend & ROI"><div className="text-[13px]" style={{ color: T.sub }}>No marketing spend loaded — the PT2 Marketing workbook ("Marketing Spend COGS …" tab) didn't return rows.</div></Panel>);
  const rows = res.unattr ? [...res.paid, { ...res.unattr, name: "Unattributed" }] : res.paid;
  const costLabel = (withLabor ? "Costs: Vendor + Labor" : "Costs: Vendor only") + (estimate && res.total.est > 0.5 ? " · incl. estimates" : "") + (attribution === "cohort" ? " · lead cohort" : "");
  const ltvLabel = `LTV: ${ltvBasis === "source" ? "each source's own" : "company"} avg fee · ${res.ltvMode === "T12" ? "trailing 12 mo" : "YTD"} (fixed)`;
  const funnelCols = [
    { key: "cost", label: "Spend", f: (r) => mkt$0(r.cost) },
    { key: "leads", label: "Leads", f: (r) => mktN(r.leads), muted: true, need: ["leads"] },
    { key: "cpl", label: "CPL", need: ["leads"], f: (r) => mkt$(r.cpl), tip: "Spend ÷ new leads (Lead Create Date)" },
    { key: "cplB", label: "Blended CPL", need: ["leads", "revives"], f: (r) => mkt$(r.cplBlended), tip: "Spend ÷ (new leads + Dead → New Lead revives), each lead once" },
    { key: "opps", label: "Opps", f: (r) => mktN(r.opps), muted: true, need: ["opps"] },
    { key: "cpo", label: "Cost / opp", f: (r) => mkt$(r.cpo), need: ["opps"] },
    { key: "arips", label: "ARIPs", f: (r) => mktN(r.arips), muted: true, need: ["arip"] },
    { key: "cpa", label: "Cost / ARIP", f: (r) => mkt$0(r.cpa), need: ["arip"] },
  ];
  const unitCols = [
    { key: "cost", label: "Spend", f: (r) => mkt$0(r.cost) },
    { key: "cust", label: "Customers", need: ["closed"], f: (r) => (r.core || r.listing ? `${r.core}${r.listing ? ` + ${r.listing} L` : ""}` : "—"), muted: true, tip: "Core sellers (Wholesale / Purchase, a front-end + back-end = one) + Listing sellers" },
    { key: "cac", label: "CAC", need: ["closed"], f: (r) => mkt$0(r.cac), tip: "Spend ÷ core customers" },
    { key: "cacB", label: "Blended CAC", need: ["closed"], f: (r) => mkt$0(r.cacBlended), tip: "Spend ÷ (core + Listing customers)" },
    { key: "ltv", label: "LTV", need: ["closed"], f: (r) => (r.ltv == null ? "—" : `${mkt$0(r.ltv)}${ltvBasis === "source" ? ` (n=${r.ltvN})` : ""}`), muted: ltvBasis !== "source" },
    { key: "ltvcac", label: "LTV:CAC", need: ["closed"], f: (r) => mktX(r.ltvCac) },
    { key: "rev", label: "Closed rev", need: ["closed"], f: (r) => mkt$0(r.closedRev), muted: true },
    { key: "roas", label: "ROAS actual", need: ["closed"], f: (r) => mktX(r.roas), tip: "Closed revenue (incl. Listing) ÷ spend" },
    { key: "open", label: "Open pipeline", need: ["closed"], f: (r) => (r.openRev ? `${mkt$0(r.openRev)}` : "—"), muted: true, tip: "Open deals, Total Forecasted Revenue, Close Date through the end of the selected period (unweighted)" },
    { key: "roasF", label: "ROAS forecast", need: ["closed"], f: (r) => mktX(r.roasFc), tip: "(Closed + open pipeline) ÷ spend" },
  ];
  const t = res.total;
  return (<Panel title={`Spend & ROI — ${costLabel}`}>
    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mb-3">
      <MktSeg label="Include labor" value={withLabor} onChange={setWithLabor} options={[[true, "On"], [false, "Off"]]} />
      <MktSeg label="LTV basis" value={ltvBasis} onChange={setLtvBasis} options={[["company", "Company avg"], ["source", "By source"]]} />
      <MktSeg label="Attribution" value={attribution} onChange={setAttribution} options={[["event", "By event date"], ["cohort", "By lead cohort"]]} />
      <MktSeg label="Unposted costs" value={estimate} onChange={setEstimate} options={[[true, "Estimate"], [false, "Actuals only"]]} />
    </div>
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
      {[["Attributed spend", mkt$0(t.cost), (withLabor && t.labor ? `vendor ${mkt$0(t.vendor)} · labor ${mkt$0(t.labor)}` : "vendor spend") + (t.est > 0.5 ? ` · ${mkt$0(t.est)} est.` : "")],
        ["Blended CPL", mkt$(t.cpl), `${mktN(t.leads)} new leads · paid sources`],
        ["ROAS actual", mktX(t.roas), `${mkt$0(t.closedRev)} closed`],
        ["ROAS forecast", mktX(t.roasFc), attribution === "cohort" ? "n/a in cohort view" : `+ ${mkt$0(t.openRev)} open pipeline`],
        ["LTV (avg fee)", mkt$0(res.ltvCompany), `${res.ltvMode} · ${res.ltvN} customers, fixed`]].map(([l, v, sub]) => (
        <div key={l} className="rounded-xl p-3" style={{ background: T.track }}>
          <div className="text-[11px] uppercase tracking-wide" style={{ color: T.faint }}>{l}</div>
          <div className="text-[22px] font-bold leading-tight" style={{ color: T.ink, fontVariantNumeric: "tabular-nums" }}>{v}</div>
          <div className="text-[11px]" style={{ color: T.faint }}>{sub}</div></div>))}
    </div>
    <SubHead label="Cost per funnel stage" note={attribution === "cohort" ? "lead cohort · leads created in the range, followed through their Opp ID to opps / ARIPs / closed deals to date" : "each count on its own date · new leads by Create Date · opps by Created Date · ARIPs by ARIP entry"} />
    <div className="mt-2 mb-5"><MktTable cols={funnelCols} rows={rows} total={t} openKey={openKey} setOpenKey={setOpenKey} withLabor={withLabor} /></div>
    <SubHead label="Unit economics & ROAS" note={ltvLabel} />
    <div className="mt-2"><MktTable cols={unitCols} rows={rows} total={t} openKey={openKey} setOpenKey={setOpenKey} withLabor={withLabor} /></div>
    {res.unpaid.length > 0 && (<div className="text-[12px] mt-3" style={{ color: T.sub }}>
      <b>No attributed spend:</b> {res.unpaid.map((r) => `${r.name} ${mkt$0(r.closedRev)} closed${r.openRev ? ` + ${mkt$0(r.openRev)} open` : ""}`).join(" · ")}</div>)}
    {res.flags.length > 0 && (<div className="flex flex-col gap-1 mt-3">
      {res.flags.map((f, i) => (<div key={i} className="text-[11px] rounded-md px-2 py-1" style={{ background: f.tone === "warn" ? T.warnSoft : T.track, color: f.tone === "warn" ? T.warn : T.sub }}>{f.tone === "warn" ? "⚠ " : "ⓘ "}{f.text}</div>))}
    </div>)}
    <div className="text-[11px] mt-3 leading-relaxed" style={{ color: T.faint }}>
      Spend is QuickBooks vendor spend mapped by the <b>Vendor x Lead Source</b> tab — Software/Service vendors go straight to their source; <b>Data</b> vendors are split by each source's Software+Service spend over the trailing {MKT_COST.weightMonths} months. Labor is Rippling contractor pay for <b>Marketing Spend - Roster</b> employees (title "Call Center" → Call Center, "Texting" → Text Message Campaign), spread over each pay period.
      Customers merge a front-end with its back-end; a back-end whose front-end closed earlier adds revenue but no new customer. LTV is the average fee per core customer, fixed at the {res.ltvMode === "T12" ? "trailing-12-month" : "year-to-date"} value; Listing is excluded from LTV but its revenue counts in ROAS. $0 front-ends awaiting their back-end are left out of LTV{res.ltvUnrealized ? ` (${res.ltvUnrealized} now)` : ""}. Click a row for its cost components.
    </div>
  </Panel>);
}
...
function ExecutiveDashboard({ store, dir, org: rawOrg, range, rangeFwd, view }) {
  ...
  const [oneOnOne, setOneOnOne] = useState(false); // "1-on-1 format" toggle: the consolidated Focus view. Off by default.
  const [txSub, setTxSub] = useState("coordination"); // Transactions view sub-tabs: coordination | dispositions
  const [mktSub, setMktSub] = useState("funnel"); // Marketing view sub-tabs: funnel (lead funnel) | spend (Spend & ROI)
  ...
  // "1-on-1 format" is an opt-in layout: when the toggle is on AND the scope is a VP or AM/FU team/rep, the
  // consolidated Focus section replaces the default grids. Off by default, so every scope shows the classic
  // grids/panels unless the user switches to the 1-on-1 view.
  const focusEligible = !isMktView && !isTxView && (isVpScope(dir, org) || isAmFuScope(dir, org));
  const vpDrill = oneOnOne && focusEligible && isVpScope(dir, org);
  const amFuDrill = oneOnOne && focusEligible && isAmFuScope(dir, org);
  ...
  const mktSubToggle = isMktView ? (
    <div className="inline-flex rounded-xl p-0.5 self-start" style={{ background: T.track }}>
      {[["funnel", "Lead Funnel"], ["spend", "Spend & ROI"]].map(([v, l]) => (
        <button key={v} onClick={() => setMktSub(v)} className="text-[13px] font-medium px-3 py-1.5 rounded-md transition-colors whitespace-nowrap"
          style={{ background: mktSub === v ? T.card : "transparent", color: mktSub === v ? T.ink : T.sub, boxShadow: mktSub === v ? "0 1px 2px rgba(0,0,0,0.06)" : "none" }}>{l}</button>))}
    </div>) : null;

  if (isMktView && mktSub === "spend") return (
    <div className="flex flex-col gap-5">{mktSubToggle}<MarketingEfficiency store={store} range={range} rangeFwd={rangeFwd} /></div>);
  ...
  return (<div className="flex flex-col gap-5">
    {txSubToggle}{mktSubToggle}
    {(!isTxView && !isMktView) ? (<>
      {focusEligible && (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[11px] font-medium uppercase" style={{ color: T.faint, letterSpacing: "0.05em" }}>Format</span>
          <div className="inline-flex rounded-lg p-0.5" style={{ background: T.track, border: `1px solid ${T.border}` }}>
            {[["default", "Default"], ["one", "1-on-1 format"]].map(([v, l]) => {
              const on = (v === "one") === oneOnOne;
              return (<button key={v} onClick={() => setOneOnOne(v === "one")} className="lh-tab rounded-md text-[12px] font-semibold" style={{ padding: "4px 12px", background: on ? T.accentSoft : "transparent", color: on ? T.accent : T.sub }}>{l}</button>);
            })}
          </div>
          {oneOnOne && <span className="text-[11px]" style={{ color: T.faint }}>consolidated one-on-one review · classic grids hidden</span>}
        </div>)}
      {vpDrill && <VpFocus store={store} dir={dir} org={org} range={range} rangeFwd={rangeFwd} drillLabel={drillLabel} results={results} />}
      {amFuDrill && <AmFuFocus store={store} dir={dir} org={org} range={range} rangeFwd={rangeFwd} drillLabel={drillLabel} results={results} />}
      {!vpDrill && !amFuDrill && <SummaryStrip items={["closed_revenue", "pipeline_forecast", "deals_closed", "show_rate"].map((id) => ({
      ...
      {!orgFiltered && vpAttribution.length > 0 && (() => {
    ...
    {repConversion.length > 0 && (() => {
      // Conversion by rep, scoped to the current selection: All view → everyone; a team → that team's reps;
      // a single rep → that rep's team. Rendered at the bottom, under Revenue & pipeline.
      let scopeSet = null;
      if (orgFiltered) {
        if (org.rep !== "All") { const tm = dir.byRep[String(org.rep).trim()]?.team;
          scopeSet = tm ? new Set((dir.people || []).filter((p) => p.team === tm).map((p) => String(p.rep).trim())) : new Set([String(org.rep).trim()]); }
        else scopeSet = repsInScope(dir, org);
      }
      const convRows = repConversion.filter((r) => !scopeSet || scopeSet.has(String(r.rep).trim()));
      if (!convRows.length) return null;
      const convLabel = (orgFiltered && org.rep !== "All") ? (dir.byRep[String(org.rep).trim()]?.team || org.rep) : drillLabel;
      return (
        <Panel collapsible title={`Conversion by rep — ${convLabel}`}>
          <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
...
export default function App() {
  ...
  const shell = (body) => (<div className="min-h-screen w-full" style={{ backgroundColor: T.canvas, backgroundImage: `linear-gradient(${T.gridLine} 1px, transparent 1px), linear-gradient(90deg, ${T.gridLine} 1px, transparent 1px)`, backgroundSize: "44px 44px", ...FONT, "--lh-accent": T.accent, "--lh-border-strong": T.border2, "--lh-row-hover": T.rowHover, "--lh-hover-shadow": T.hoverShadow }}>
    <style>{`
      ...
      .lh-frozen{max-height:70vh;overflow:auto;position:relative;-webkit-overflow-scrolling:touch}
      @media print{.lh-frozen{max-height:none!important;overflow:visible!important}.lh-frozen thead th,.lh-frozen tbody td{position:static!important}}
