...
function UnderwritingView({ store, range, dir, org }) {
  ...
  const acc = useMemo(() => {
    ...
    const scored = []; let closeMatched = 0, flipNoArv = 0, inWindow = 0, skippedNoBasis = 0;
    accRows.forEach((r) => {
  ...
  return (
    <div className="flex flex-col gap-5" id="underwriting-view">
      ...
      <Panel title="Underwriting accuracy — valuation delta on closed deals">
        ...
        <div className="text-[11px] mt-3" style={{ color: T.faint }}>Basis = the first "As Is Valuation" column (falls back to the second when blank). Back-End flips use the front-end opp's ARV matched by name{acc.flipNoArv > 0 ? ` (${acc.flipNoArv} flip${acc.flipNoArv === 1 ? "" : "s"} skipped — no front-end ARV found)` : ""}. {acc.closeMatched} of {acc.total} rows have a matched close date; deals without one appear only under the All-time window.</div>
      </Panel>
