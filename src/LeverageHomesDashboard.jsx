...
function amFuMetricsFor(store, dir, org, range, rangeFwd) {
  ...
  return { arips, setGroup, attByType, aripToDR, pipeline, closedRev, calls, extra };
}
...
function VpPerVpTable({ perVp, groups }) {
  ...
  return (<div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
    <table className="w-full text-[13px]" style={{ borderCollapse: "collapse", tableLayout: "fixed", minWidth: 300 + nVp * 120 }}>
      <colgroup><col style={{ width: `${100 - nVp * 22}%` }} />{perVp.map(({ vp }) => <col key={vp} style={{ width: "22%" }} />)}</colgroup>
      <thead><tr>
          ...
          <React.Fragment key={group.g}>
            ...
            {group.rows.map((r, ri) => (
              <tr key={r.h} className="lh-row" style={{ color: T.ink, background: ri % 2 ? T.zebra : "transparent" }}>
                <td className="py-2.5 px-3.5" style={{ ...cell, color: T.ink, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.h}</td>
                {perVp.map(({ vp, m }) => (
