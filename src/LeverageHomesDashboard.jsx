          ...
          <React.Fragment key={group.g}>
            ...
            {group.rows.map((r, ri) => (
              <tr key={r.h} className="lh-row" style={{ color: T.ink, background: ri % 2 ? T.zebra : "transparent" }}>
                <td className="py-2.5 px-3.5" style={{ ...cell, color: T.sub, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.h}</td>
                {perVp.map(({ vp, m }) => (
